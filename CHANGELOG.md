...
- **The 1NT book's 21 build-time knobs are carried, not fetched.**
  `Build::notrump` captures every build-only cell in
  `american/notrump{,/**}.rs` once through `notrump::capture()`, and the rule
  builders read the resulting `NotrumpKnobs` fields: Stayman and its slam
  tries, the transfers, Texas, the splinters, the size ask, the six-card
  invitations and the minor schemes.  Derived readings stay functions,
  including the minor-scheme selectors, the six-card-invite and Texas floor
...
- **The port checklist, batch by batch.** One rolling entry for the campaign —
  ...
  were pairwise distinct and byte-identical across it. T1 repeated the final
  31-arm proof, then retired the campaign harness. User impact: none.
  ...
  - **Z1** — `raises.rs`'s seven `insert_uncontested` sites and three
    ...
    User impact: none.
  - **T1** — the three RKCB test fixtures now compile `rkcb_rows` directly from
    their row-native prefixes, and the test-only `install_rkcb` shim is gone.
    Active comments and Dutch planning docs name the producer rather than the
    retired installer; historical CHANGELOG references remain historical. The
    final 31 sweep arms are still pairwise distinct and byte-identical to Z1,
    `smoke-default` remains
    `59a27d7f36bf4bd1b1026825769d8f26bec14f030367c141fb299e72073217dc`,
    and `render-book` remains
    `01bf875f750069bab23e250b2ab6324f852cf2b79d0bb4c4aa2a2c546c5d4abc`.
...
- **Book assembly is becoming declarative: a new row layer
  ...
  parens, a leading `P*` fanning the 0–3 leading-pass seats — with seat
  alternation validated at build time; a `Row` binds one pattern to one rule,
  and a `Package` (name, knob gate, entries) is the unit of assembly.
  ...
  `competition()`); Section 11 (Jordan/Truscott) in the fine-grained form —
  one `row(...)` per rule with per-row alerts, the four wiring-only table
  fns dissolved into the package, and the shared cue-raise answers lifted
  whole via `rows_of`; and the weak-two defense in `defensive()` as the
  exact-node pilot. Two invariants are now machine-checked per
...
- **`set_fuzzy_strength` and `set_fuzzy_points` are deleted.** The first wrote
  *two* sibling cells, so flipping it silently moved a knob the caller never
  named; the second was a bool over the three-valued `PointScale`, unable to
...
- **`american()` is now floored off-book by the BBA-distilled neural net** (was
  ...
  disclosable-only v3 features and forced-rail safety shell as
  `american_neural_v3()`, but is distilled from the vendored **BBA / EPBot 2/1
  oracle** — a ~1.9-IMP/bd-stronger teacher than `american()` itself (held-out
  top-1 agreement with BBA **85.9%**; hard one-hot/argmax targets over
  disclosable-only features, less learnable than american's 95.3%, as expected).
  **Routing-gate A/B vs the prior `instinct` floor — live BBA opponents,
  dual-scored, two independent seeds (~150k bd/cell total): +0.11 IMPs/bd
  non-vul, +0.25 IMPs/bd vulnerable — plain-win AND PD-win in all four cells,
  3.5–9σ** (vs `neural-v3` the teacher effect is larger, +0.32/+0.47). No
  doubling artifact (PD ≥ plain); disclosure audit clean — call distribution
  matches the old floor's, no new artificial-call class, the
  `artificial_calls_are_alerted` book invariant untouched, no book nodes added.
  The deterministic system is preserved as `american_instinct()` (see Added);
  `american_bba_neural()` is now an alias of `american()`. The BBA net path is
  always compiled; the `neural-floor` feature still gates the legacy
  v1/v2/v3/search variants. Also adds `dump-teacher --teacher american|bba`
