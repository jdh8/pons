#!/bin/sh
# ab-takeout-support.sh — A/B for the anchor's #1 bucket (Defensive/book/round-1).
...
#
#   setsid nohup scripts/idle-run.sh scripts/ab-takeout-support.sh \
...
R=${1:?usage: ab-takeout-support.sh RESULTS_DIR}
SHOW=3
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)
