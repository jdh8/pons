#!/bin/sh
# reading-drift-ab.sh — the batched A/B for the reading-drift repairs
...
#
#   setsid nohup scripts/idle-run.sh scripts/reading-drift-ab.sh \
...
set -eu
R=${1:?usage: reading-drift-ab.sh RESULTS_DIR}
cd "$(dirname "$0")/.."
mkdir -p "$R"
