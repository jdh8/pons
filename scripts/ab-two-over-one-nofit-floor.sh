#!/bin/sh
# ab-two-over-one-nofit-floor.sh — is the shipped 13-HCP no-fit 2/1 floor too
...
#   setsid nohup scripts/idle-run.sh scripts/ab-two-over-one-nofit-floor.sh \
...
set -eu
R=${1:?usage: ab-two-over-one-nofit-floor.sh RESULTS_DIR}
COUNT=${COUNT:-1000000}
