#!/bin/sh
# ab-sputnik-negx.sh — four-arm negative-double A/B adding the Sputnik residual
...
# ab-competitive-book.sh: one SEED_BASE for the experiment (paired diffs need
# identical deals), arms strictly sequential, both scorers, both vulnerabilities.
#
#   setsid nohup scripts/idle-run.sh scripts/ab-sputnik-negx.sh \
...
R=${1:?usage: ab-sputnik-negx.sh RESULTS_DIR}
SHOW=3
. "$(dirname "$0")/ab-lib.sh"
