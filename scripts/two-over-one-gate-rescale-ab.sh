#!/bin/sh
# two-over-one-gate-rescale-ab.sh — re-probe the major no-fit 2/1 gate under the
...
#   setsid nohup scripts/idle-run.sh scripts/two-over-one-gate-rescale-ab.sh \
...
set -eu
R=${1:?usage: two-over-one-gate-rescale-ab.sh RESULTS_DIR}
