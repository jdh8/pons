#!/bin/sh
# advance-sit-hcp-ab.sh — sweep the advancer's 4-card penalty-pass quality
# gate over partner's takeout double ([1t, X, P]).  The shipped gate is two
...
# sequential; modeled on scripts/advance-pass-yield-ab.sh; do NOT touch the
# codebase while it runs.
#
#   setsid nohup scripts/idle-run.sh \
#       scripts/advance-sit-hcp-ab.sh ab-results/advance-sit-hcp \
...
R=${1:?usage: advance-sit-hcp-ab.sh RESULTS_DIR}
