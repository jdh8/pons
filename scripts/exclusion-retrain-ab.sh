#!/bin/sh
# exclusion-retrain-ab.sh — pass-exclusion re-measured behind its evaluator twin.
#
...
# knob stays opt-in.
#
#   setsid nohup scripts/idle-run.sh scripts/exclusion-retrain-ab.sh \
...
R=${1:?usage: exclusion-retrain-ab.sh RESULTS_DIR}
SHOW=${SHOW:-40}
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)
