#!/bin/sh
# net-collar-ab.sh — collar the bilans net instead of letting it replace the
...
#
#   setsid nohup scripts/net-collar-ab.sh ab-results/net-collar \
#       >ab-results/net-collar.log 2>&1 &
#
# Resumable; SEED_BASE persists in $R/seed (a NEW dir -> a new seed).
R=${1:?usage: net-collar-ab.sh RESULTS_DIR}
SHOW=${SHOW:-40}
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)
