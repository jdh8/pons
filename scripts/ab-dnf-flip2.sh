...
#   off   --no-ns-envelope-union  (the legacy hull reading, pre-flip default)
#   dnf   the shipped default (union-of-boxes + knob-matched evaluator)
#
#   setsid nohup scripts/idle-run.sh scripts/ab-dnf-flip2.sh \
#       ab-results/dnf-flip2 >ab-results/dnf-flip2.log 2>&1 &
#
# Resumable; SEED_BASE persists in $R/seed (a NEW dir -> a new seed).
R=${1:?usage: ab-dnf-flip2.sh RESULTS_DIR}
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)

log "=== dnf flip2 start, sha=$SHA, SEED_BASE=$SEED_BASE, ${SHARDS}x${PER_SHARD} bd/arm/vul (chop F2b: DNF-on + evaluator_v2_dnf vs --no-ns-envelope-union)"
for vul in none both; do
    arm off "$vul" --no-ns-envelope-union
    arm dnf "$vul"
