#!/bin/sh
# closure-isolate-ab.sh — why did chop C1 lose?
#
# `scripts/closure-ab.sh` measured the `Σ len = 13` closure at −0.037/−0.046
...
#
#   setsid nohup scripts/closure-isolate-ab.sh ab-results/closure-isolate \
#       >ab-results/closure-isolate.log 2>&1 &
R=${1:?usage: closure-isolate-ab.sh RESULTS_DIR}
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)
