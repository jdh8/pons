#!/bin/sh
# ab-multi-2d.sh — the Multi 2♦ opening knob (`opening.multi_two_diamonds`) on
# american, both variants, one run.
#
# Three arms, all `--our-floor american` against the same BBA reference
# opponent on the same deals, so each comparison is paired and the only moving
# part is the 2-level opening table:
#
#   plain      american with its three natural weak twos       (control)
...
#       scripts/ab-multi-2d.sh ab-results/multi-2d \
#       >ab-results/multi-2d.log 2>&1 &
#
...
# its own seed.
R=${1:?usage: ab-multi-2d.sh RESULTS_DIR}
...
log "=== multi 2♦ start, sha=$SHA, SEED_BASE=$SEED_BASE, ${SHARDS}x${PER_SHARD} bd/arm/vul"
for vul in none both; do
    arm plain    "$vul" --our-floor american
    arm multi    "$vul" --our-floor american --ns-multi-2d --no-ns-multi-2d-champion
    arm champion "$vul" --our-floor american --ns-multi-2d
