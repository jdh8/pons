#!/bin/sh
# ab-announced-reading.sh — split disclosure from projection, and measure the
...
#
#   setsid nohup scripts/idle-run.sh scripts/ab-announced-reading.sh \
...
R=${1:?usage: ab-announced-reading.sh RESULTS_DIR}
SHOW=${SHOW:-40}
. "$(dirname "$0")/ab-lib.sh"
SEED_BASE=$(seed_for)
