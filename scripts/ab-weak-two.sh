#!/bin/sh
# ab-weak-two.sh — fix-vs-shipped A/B for the weak-two remnant (Root A):
...
#   setsid nohup scripts/idle-run.sh scripts/ab-weak-two.sh \
...
set -eu
R=${1:?usage: ab-weak-two.sh RESULTS_DIR}
