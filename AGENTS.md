...
| Neural/AI bidder work | `ai-bidder` skill + [docs/ai-bidder/](docs/ai-bidder/) (`README.md` then `plan.md`) |
...
softmax/logits already live in `src/bidding/array.rs`). Delegate bounded,
independent tasks to subagents; keep design and integration in the main loop.
Use cheaper subagents for repetitive, well-specified edits (the same change
across N files, bulk ledger rows), one file per task, and review every diff.
...
distillations, rules) belong in these checked-in docs; Codex thread context
holds only transient state (in flight, owed) plus pointers into the docs.
