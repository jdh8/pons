...
| `their_landy_reading` | **their disclosed** Landy 2♣ over our 1NT (`their.two_clubs_landy` + `reading.their_landy_reading`, added 2026-08-14) | cannot retire into our projection: their calls have no authored rule of ours to project. Its chop is the declared-opponent `their_profile` split (decode their 2♣ off their own declared book — the 2b reader half declarative-rows.md keeps open); until then it is the seat-gated twin of the stub above |
| `their_multi_reading` | **their disclosed** Multi 2♦ over our 1NT (`their.two_diamonds_multi` + `reading.their_multi_reading`, shipped 2026-08-16) | same declared-opponent-profile blocker: the temporary reader supplies exactly `{ ♥6+ } ∪ { ♠6+ }` and suppresses the first pass-or-correct advance until their authored profile can project both calls; see ledger 6 |
...
| 6 | `their_multi_reading` | Added as the disclosed-opponent twin, not a candidate chop: first opposing action only, exact two-box `6+♥ ∪ 6+♠` union in actual and announced inference, natural-2♦ and first `2M` pass-or-correct readings suppressed, systems-on strip fenced. No strength/minor/other-major claim | **SHIPPED DEFAULT-ON 2026-08-16.** Three independent 230.4k-board seeds per vulnerability, zero foreign boards: NV plain −0.0001 ±0.0003 / PD +0.0004 ±0.0004; vulnerable plain +0.0001 ±0.0003 / **PD +0.0006 ±0.0004**; both-vulnerability pool `plain wash \| PD win`. Retirement blocker is explicit: delete this row only when declared-opponent profiles project their own rules |
