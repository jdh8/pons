...
### N1q — the two-level majors sorted by **strength**, not by shortness (`defense_2c_landy_strength_majors` + `_doubles`, **measured twice 2026-09-18: first build a loss, flip arm a non-win; both knobs stay off**)

Knobs `competition.defense_2c_landy_strength_majors` and
`competition.defense_2c_landy_strength_doubles` (`bba-gen --ns-landy-strength`
/ `--ns-landy-strength-doubles`; `render-book --ns-landy-responder
strength|strength-doubles`; `probe-call-reading` carries both).  Code:
...
* **The pre-registered falsifier fired, both-vul only.**  `X → 2♠` is −7,409
  plain both-vul, and all of it is opener's `2NT`@150 answer: −7,753 on 4,845
  boards, whichever way responder continues (pass −1.81/fired, pull to `3♣`
  −1.62, to `3♦` −1.43), while the direct picks read +1.00 (`3♦`) / −0.68
  (`3♣`).  The donors are the 1-4-4-4s (−1.87/fired) and the 1-3 shapes
  (−0.72); the 5-5s and 0-4s win.  Non-vulnerable the identical continuation
  is −1,200 / −1,025 / −704 against picks of +839 / +1,870 — the 8-9 takeout
  is worse than the values double it displaced exactly when opener has both
  majors stopped and we are vulnerable.  The pre-registered consequence
  stands: the band retreats to ten-plus, which is N1j's takeout merged and a
  knob not worth keeping.
* **Fix (c) over-corrected.**  `2♥ → 2♠` *grew*, to −6,290 / −2,792 plain, and
  it is now the 10-12 band: `2♠ - 2NT - 3NT` (responder's raise on ten)
  −3,251 / −2,136 at **−4.25 / −1.89 per fired**, and `2♠ - 3♣ - -` (the pass
  under the thirteen-point `5m` gate) −1,823 / −883.  The 8-9 half of the
  cell is flat (−36 both-vul, +759 NV).  N1j's shortness-sorted ten-plus
  takeout routes those 25-28 combined to game; the flip arm raises `2NT` to a
  failing `3NT` or stops in `3m`.
* **The PD deficit is the weak rung, whole.**  Class `2♥ - 3♣` is PD
  **−20,624 both-vul / −25,128 NV** (`- → 2♥` PD −16,142 / −9,537, `2♦ → 2♥`
  −2,744 / −8,997) while plain wins the same boards: a five-to-seven count at
  the three level opposite 15-17 is the doubling-artifact row's textbook case,
  and no continuation repairs a contract that is simply too high.  Run 1's
  "falsifier 1 did not fire" was a plain-DD statement; PD says the weak rung
  is the thing that loses.  The contested tail `2♥ (3♥) X` — opener's floor
  double — adds −3,883 / −772 plain and −10,850 / −7,962 PD on ~1,300 boards
  (`strx`'s takeout `X` is the +0.0002 that cell recovers).
...
**Not built — the user's call.**  Two third-arm ideas fall out of the cuts:
(i) colour-gate the 8-9 half of `2♠` to non-vulnerable, where the residual
`X → 2♠` reads +410 plain / +6,148 PD; (ii) route ten-plus through N1j's
answer tables (the falsifier's own "retreat").  Together they reduce the knob
to "a non-vulnerable 8-9 both-minor takeout plus the weak `2♥`", and the weak
`2♥` is the PD loser — so the surviving content does not clear a default-on
bar, and §N1q closes here as **measured twice, opt-in, default byte-identical**.

#### Owed

* ~~The flip arm~~ — built and **measured 2026-09-18/19, a non-win**; verdict
  above.  Both knobs stay off; the two third-arm ideas above are not queued.
