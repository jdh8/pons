...
### N1q — the two-level majors sorted by **strength**, not by shortness (`defense_2c_landy_strength_majors` + `_doubles`, **built 2026-09-18, both default off, A/B owed**)
...
Responder: `2♠`@177 `both_minors & points(8..)` (`comp:landy-minors-inv`),
...
Opener over `2♠`: `3NT`@160 (`landy_lia_accept`, the shared maximum-with-both-
stoppers gate), `2NT`@150 on both stoppers, `3♣`@100 / `3♦`@99, `3♣`@0.
...
Responder over `2♠ - 3m -` reuses `landy_lia_pick_rebid` verbatim.  Sit rails
(`Pass`@0) at `2♥ - 3m -`, `2♠ - 3NT -` and `2♠ - 2NT - 3m -`, per the lia2
...
* The A/B: `scripts/ab-landy-strength.sh`, three arms sequential on one fresh
  `SEED_BASE`, NV and both-vul, plain DD + PD, sd bracket if the headline earns
  it.
