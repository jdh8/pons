...
### N1r — idea queue for `1NT (2♣) 3♦`+ (**collected 2026-09-21; step 0 — `3NT` vs the values `X` by colour — SHIPPED DEFAULT-ON 2026-09-22 at favourable; step 0b — the misfit-only gate — MEASURED LOSS 2026-09-22, dropped; the queue itself unbuilt**)
...
would misread the short-spade jam.  **Proposed reversible default: close
both, unbuilt**, and treat §N1r as finished bar arm 2's judgment call.
...
| **N1m** opener's own rebid over their advance | `competition.landy_opener_px`, `landy_opener_rungs` (**both ON**, off-switches `--no-ns-landy-opener-px` / `--no-ns-landy-opener-rungs`) | **SHIPPED DEFAULT-ON 2026-09-16 as a package; `px` alone refused** | `px + rungs vs base` (seed `1789547524`, sha `00c0421e`, 4.608M bd/arm/vul, all six gates 0 foreign): plain DD **+0.0140** none / **+0.0220** both, DD-PD +0.0251/+0.0341, sd-plain +0.0004/+0.0030, **SD-PD +0.0095/+0.0135** — non-negative on all four scorers at both colours. **`px` alone is not shippable**: plain DD likes it (+0.0099/+0.0249) and the sd bracket refuses it (**−0.0234 sd-plain, −0.0075 SD-PD** white), because its `Pass`@0 catch-all is "passed where the baseline bid" on **85.8%** of divergent boards — the §N1l-flip catch-all defect at the seat one call earlier. **Falsifier 3 answered affirmative**: `rungs vs px` is +1.819/+1.923 IMPs/fired sd-plain and positive SD-PD at both colours while DD reads ≈0, so the rungs carry the package on the honest-lead scorers and the `X`@150 above them is the ≤3-trump cap — §N1k's geometry is genuinely repaired, not merely capped. The oracle's DD-first ranking of `2Mx` did not transfer under a blind lead; it has no column for what the floor does with the rest of the auction. The seat §N1k lost at, re-opened as its own arm per flagged item 1. Designed off `probe-landy-opener-oracle` (103,653 + 81,023 seat boards, 105,334 deals solved): defending their major **doubled** wins every four-plus-trump bucket at both vuls (+2.8…+8.1 IMPs/bd over the floor, PD flat) and loses on two or three, so `len(major, 4..)` is the whole gate. `X`@150 above the notrump rungs supplies the ≤3-trump cap `has_stopper` could not — 17.4% of §N1k's gate was four-trump hands where its `3NT` forwent +7.0…+7.8. `3m`, `3OM` and the relay leg all priced out and are absent | [§N1m](#n1m--openers-own-rebid-over-their-advance-landy_opener_px--landy_opener_rungs-shipped-default-on-2026-09-16-as-a-package); `scripts/ab-landy-opener.sh` |
...
| **N1q** the two-level majors sorted by strength | `competition.defense_2c_landy_strength_majors` (**on since 2026-09-20**), `defense_2c_landy_strength_doubles` and `landy_recue_signoff` (**both on since 2026-09-21**, run 4: rail plain +0.0012 / +0.0009, doubles +0.0000 / +0.0001 with sd positive 4/4), `defense_2c_landy_strength_nv_invite` (**on since 2026-09-21**, run 5: plain +0.0018 ±0.0003 both-vul, PD wash, sd-lead plain +0.0013 ±0.0004, NV 0 divergent) | **SHIPPED DEFAULT-ON 2026-09-20** on the third arm (`Pass` rails at the floor-phantom nodes + opener's `3NT` over their raise of `2♠` gated at seventeen): `str` vs base plain **+0.0078 NV / +0.0050 both-vul**, PD **+0.0070 / +0.0026**, sd-lead +0.0096 / +0.0075 plain, +0.0087 / +0.0054 PD — win\|win at both colours, 4.608M bd/arm/vul; falsifier 2 fired inside the win (vulnerable `X → 2♠` −8,563 plain), so the colour gate on the 8-9 band is the next arm; `strx` NV +0.0004 plain, both-vul wash, stays off with two unauthored tails (`2♥ (2♠) X (XX)`, `2♥ (2♠) X - 3m (3♠)`). History: run 1 `str` vs base plain **−0.0029** both-vul / +0.0056 NV, PD −0.0192 / −0.0115; run 2 (the flip arm) plain +0.0006 both-vul / **+0.0067** NV, PD **−0.0076 / −0.0031** (sd-lead +0.0050/+0.0109 plain, −0.0023/+0.0026 PD) — NV is the win\|loss doubling-artifact row, both-vul wash\|loss, and the pre-registered falsifier fired both-vul (`X → 2♠` −7,409 plain, all through opener's `2NT`); `strx` vs `str` +0.0002/+0.0003 plain, moot on a non-winning parent. Falsifier 2 fired, not 1: the loss is `X → 2♠` (−16,865 plain both-vul), monotone in the short major, routed through opener's `3NT`@160/`2NT`@150 answers; two secondary cells (`2♥ → 2♠` with no authored rebid over the `3m` pick, `3♣ → 2♠` shadowing the diamond transfer). The flip arm (short major and no six-card minor at 8-9, no `3NT` accept, `5m` on thirteen) cured both secondaries and the NV half of `X → 2♠`, and the scored re-solve (2026-09-19) locates the residual in **three families of floor phantoms at unauthored nodes** — our `3NT` in the weak rung's contested tails (PD −27k / −28k, the whole deficit; the quiet `3m` is PD-neutral), opener's `4♣` over the *making* `3NT` raise at `2♠ - 2NT - 3NT -`, responder's `3NT` at 8-9 over their raise — which sat (`scripts/divergence-sit.py`) read +0.0075 / +0.0055 NV and +0.0029 / −0.0005 both-vul; the third arm of `Pass` rails is what shipped. What stays structural is the vulnerable 8-9 `X → 2♠` against §N1m's penalty double of their advance; verdicts below | N1j spends both two-level majors naming *which* major is the doubleton and gates both at `points(10..)`, so the weak both-minor hand and the invitational four-four hand have no call at all. §N1q re-sorts them by strength: `2♠`@177 is the whole 4+♣4+♦ INV+ band (four-four allowed, `points(8..)`, unlimited above, `comp:landy-minors-inv`) above the transfers; `2♥`@141 is the weak five-four band down beside the `2♦` escape it outranks, carrying the escape's `natural_floor` (`comp:landy-minors-weak`); the splinters move to 179/178 as they do under lia, since the strength rung's shape contains theirs. Neither rung claims anything about the majors, so the shortness ask and the `3M` cue are gone and both answer tables are new — minors only over `2♥` (**no notrump rung**: a `2NT` on stoppers opposite a possible five-count is lia3's both-vul catastrophe one rung over), and `3NT`@160 / `2NT`@150 / `3m` over `2♠`. **Census that sized it** (`ab-results/landy-lia3/base-none`, 4.608M bd, table A; seat reached on 17.83%): the weak 5-4 band is **1.77% of all boards** and passes 59.6% of the time today, the INV+ 4-4 band is **2.67%** and **doubles 28.0%** of the time — so the strong rung's biggest donor is the values double, which is §N1p's convicted mechanism, and the PD column is therefore **not** waved through as the auto-double artifact. The four-four weak band keeps passing by design (1.15%, 100% Pass today). **Build correction to the design sketch**: the `2NT` sign-offs were sketched `len(♣, 4..)`, which fires on every hand in a band that is 4+4+ by construction and makes the `Pass`@0 dead — both are gated at **five** cards, cheapest first. Contested split per lia2: `2♥ (2♠)` and `2♠ (3♥/3♠)` get tables, `2♥ (3♥/3♠)` goes to the floor (the sell-out class where the floor measured *right*). Second knob adds the polarities — takeout `X`@120 over their raise of `2♥`, penalty `X`@140 (`comp:landy-penalty`, `.penalty()`) over their raise of `2♠` — as explicit rules, since nothing mechanises the polarity here. Falsifiers: 1 the weak `2♥` fires and loses both-vul (flip arm = a `!vulnerable()` gate); 2 the strong `2♠` loses to the `X` it displaced (repair = a major term, not a retreat); 3 phantom suits in the floored seats after `2♠` | [§N1q](#n1q--the-two-level-majors-sorted-by-strength-not-by-shortness-defense_2c_landy_strength_majors-shipped-default-on-2026-09-20-on-the-third-arm-_doubles-and-the-landy_recue_signoff-rail-on-since-2026-09-21); `scripts/ab-landy-strength.sh`, `scripts/ab-landy-strength-residue.sh` (run 4 verdict) |
| **N1r step 0** `3NT` vs the values `X` by colour | `competition.landy_notrump_no_major_favourable` (**on since 2026-09-22**); `landy_notrump_no_major` (all colours, **off**) | **SHIPPED DEFAULT-ON 2026-09-22** at favourable only | `nt` vs base, IMPs/board DD plain / DD PD / sd-lead plain / sd-lead PD: **ew +0.0147 / +0.0127 / +0.0042 / +0.0023** (all CI-clear); ns −0.0107 / −0.0144 / −0.0206 / −0.0235; none +0.0007 / −0.0009 / −0.0108 / −0.0123; both +0.0052 / +0.0004 / −0.0054 / −0.0092. Seed 1789977169, control `ab8bc884`, 4.608M bd/arm/vul, gates 0 foreign. The DD−sd seam is +0.010…+0.012 in every cell; only favourable clears zero after it. Face-gated, byte-identical to `nt` at ew and to base elsewhere (seeded identity check), so the run is its own A/B | The colour flip §N1p did not find because it never ran the asymmetric cells. Build trap: a colour term *inside* the constraint drifts the `X`'s exclusion reading at every colour; `Rules::face` is the switch the reader honours | [§N1r step 0](#step-0-verdict--3nt-vs-the-values-x-all-four-colours-landy_notrump_no_major_favourable-shipped-default-on-2026-09-22); `scripts/ab-landy-nt-vs-x.sh` |
| **N1r step 0b** the misfit-only `3NT` gate | none — knob dropped 2026-09-22 | **MEASURED LOSS 2026-09-22**, code removed | `misfit` vs base at both, IMPs/board DD plain / DD PD / sd-lead plain / sd-lead PD: +0.0002 / −0.0065 / −0.0017 / −0.0069; vs the shipped full gate at ew: −0.0100 / −0.0118 / −0.0022 / −0.0034 (vs base at ew +0.0047 / +0.0008 / +0.0022 / −0.0008 — beats base, dominated). Seed 1789977169, control `cfa536b7`, 4.608M bd/arm/vul, gates 0 foreign | The short-major hand is the larger half of the favourable win (31,274 of 31,894 ew divergences are `X → 3NT` on a short major; the runout lands in the five-card suit). At both, 64% of divergence is opener's continuation under the widened `X` reading with responder's `X` unchanged — the reading tax of any narrowed `3NT`, invisible to the synthetic census. Both halves lose on sd-lead at both vulnerable: that colour is closed | [§N1r step 0b](#step-0b-verdict--the-misfit-only-3nt-gate-measured-loss-2026-09-22-knob-landy_notrump_no_major_misfit-and-runner-dropped-nothing-shipped) |
...
| **N1r row 5 census** quantitative `4NT` over the direct `3NT` | none — `examples/probe-landy-notrump-stopper --quant` | **CENSUS 2026-09-23, DEAD**, nothing built | Oracle on step 0's base dumps, both `3NT` rungs: 74,856 / 51,481 boards (none / both), responder 16–17 on **200 / 52**, 18+ on 0. `quant` (opener `6NT` on 17, else `4NT`) vs live −0.00001 IMPs/board; a blind `6NT` on every 16–17 `3NT` ≈ +0.00006 / +0.00001 | Responder's 16+ hands double, splinter or transfer first; the 16 + 17 pairing never occurs. Rows 4 and 8 flagged below resolution in the same pass, proposed closed | [row 5 verdict](#row-5-step-0-verdict--the-quantitative-4nt-census-2026-09-23-dead-at-step-0-responder-16-never-bids-3nt) |
