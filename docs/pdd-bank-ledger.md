...
| `22.pdd` | 3,250,000..4,160,000 | `american_bba_v8` (registered 2026-09-26): the 188 fleet chunks **re-walked at HEAD** through `features_v8` (`relabel-worker.sh` with `LAYOUTS=0`, the only walk whose per-board stream matches the fleet's), the M32 cut labels transplanted row for row by `scripts/splice-labels.py` (alignment asserted per chunk on one-hot, DD and tags). No new deals, no new rollouts; same evaluation rule — fresh deals only |
