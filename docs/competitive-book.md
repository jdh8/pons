...
- **A `their.*` disclosure drives both the book and the reading.** The N1
  ...
  ([closed §N1g](archive/one-notrump-competitive-closed.md#n1g--the-read-side-wiring-shipped-default-on-2026-08-14)).  A disclosure with no read-side
...
([closed N1b](archive/one-notrump-competitive-closed.md#n1--the-landy-2-counter-shipped-default-on-2026-08-14)): the same
...
- `set_major_support_double` — support X/XX after `1♥ - 1♠` followed by an
  overcall below 2♠ (or X), reusing `support_rules(Spades)`.
...
suits weak NF (2/1 off over X), 1NT 6–9. The `1x (X)` rebase stays
for every deeper suffix; opener continuations that the rebase would misread
get exact-suffix nodes (`2NT -` → cue-raise answers — else Jordan lands on
Jacoby 2NT; `3o -`; weak `2x -`).
...
`Cachalot` / `Sputnik` (`scripts/ab-sputnik-negx.sh`). Sputnik shares the
...
(6→7→8, `scripts/ab-free-bid-floor.sh`) then proved that leak is **structural,
...
- Balancing-seat two-suiter reading (`(1M) - - 2M/2NT`) is not recorded —
...
| P7 two-suiters over 1m | `set_uvu_over_minors` | **authored 2026-08-14, opt-in — unmeasurable vs anchor** (def1-c/d probes: 0 Michaels cues in 5000 hands/seat; the trigger never occurs, so no A/B and no default divergence). Retires the generic neg-X misbid at `1m (2m)`; minor-side half of the [closed Landy-cue analogy](archive/one-notrump-competitive-closed.md#n1--the-landy-2-counter-shipped-default-on-2026-08-14) | — (no trigger vs BBA; smoke-default byte-identical, `8ea2f567…`) |
...
`Forcing` on the existing `scripts/ab-free-bid-style.sh` — do **not** reshape the
...
`1♣ (1♠) 2♥ -` (seat 0): **Pass (NF) vs bids-again (F)**. BBA: extend
...
`scripts/ab-free-bid-style.sh`, both plain-DD and PD (sd if close), both vuls,
