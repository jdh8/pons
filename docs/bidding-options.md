...
| set_reading_scope = `All` | `--ns-reading-scope all` | Engine | **opt-in** | Projects **unalerted** (natural) authored calls too, without changing alerts or suppressing the walk. Phase 2 repaired the newly exposed soundness seams and paired 40k-board probe improved partner exclusions **2.114%→2.109%**. The ship gate nevertheless failed: across seeds **1786883097 / 1786883532 / 1786883968**, 204,800 boards/arm/vulnerability each, pooled plain DD was +0.00135/+0.00181 IMP/board NV/vul, but PD was **−0.00391/−0.00379**; PD lost at both vulnerabilities on every seed. Details: [authored-reading-handoff.md](authored-reading-handoff.md#phase-2-measurement--held). | fresh | **HELD**; retain `Alerted` as default and `All` as the diagnostic/retest arm |
