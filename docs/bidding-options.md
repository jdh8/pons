...
| `CompetitionKnobs::multi_doubler_minimum_notrump` | `--ns-multi-doubler-minimum-notrump` (enable); `scripts/ab-2d-multi-doubler-min-nt.sh`; `PROBE_MULTI_DOUBLER_MINIMUM_NOTRUMP=1` | Natural | **Off** | Extends `multi_doubler_notrump`'s ladder down to the **15-count**: `2NT`@120 on `hcp(15..) & stopper_in(major)` below the `3♠`@130 invite, plus responder's `3NT`@140 acceptance on `hcp(10..)` (the 25 opposite a known 15). **One leg only** — `2NT` outranks spades at the same level so the rung exists over `2♠`; the `3♥` leg has no room below `3NT` and its 15-count keeps passing, which is why this is its own knob rather than a one-point relaxation of the `hcp(16..)` floor. The hand it repairs is board 1 of the notrump out's worst tail (`A82.A53.KJ75.K74`, 15 HCP, three spades), pinned as *unrepaired* by `kk_doubler_notrump_repairs_the_answer_table` and as *repaired* by `kk_doubler_minimum_notrump_bids_the_fifteen_count`. `smoke-default` byte-identical when off | **A/B owed** | Smoke only (2026-08-27): 120k boards both-vul diverge on **5** — 1 in 24 000, ~1.8× the parent rung's surface — 100% "bid where the baseline passed", isolation gate **0 foreign**, 3 of 5 reaching a game the baseline never bid. The case *for*: the same rung one point higher won all four cells by 3–4 SE on 2026-08-27. The case *against*: 15 opposite `hcp 8+` is 23 combined, so this invitation can be wrong in both directions, and a negative cell is evidence the ladder should stop at 16 |
