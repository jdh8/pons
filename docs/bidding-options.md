...
| set_reading_scope = `Alerted` | `--ns-reading-scope alerted` (`ab-alert-reading`) | Engine | opt-in (default until 2026-08-16) | reads alerted call as artificial (vs `None`, the whole-projection off arm): **plain +0.0171/+0.0234, PD +0.0207/+0.0278** NV/vul (1M×2, all CI>0, PD≥plain, +2.0-3.3 IMPs/div). Refreshes the stale-PD +2.08/+1.59; contested defense-switch value is extra. Still the view `legacy_view` serves the frozen nets | fresh | superseded by `All` as default ✓ |
| set_reading_scope = `All` | `--ns-reading-scope all` (default; `alerted` is the off arm) | Engine | **ON** | Projects **unalerted** (natural) authored calls too, without changing alerts or suppressing the walk. First whole-book run (seeds 1786883097/…532/…968) was **held**: plain +0.0014/+0.0018 but PD −0.0039/−0.0038 NV/vul. Forensic (`ab-dump-bucket --by lane`) put the entire loss in the four `1x (1NT)` lanes — the side-blind systems-on strip misreading partner's negative double as a 15+ penalty double of a 1NT opening. Strip fixed (ours-only; nets held on the old view by `legacy_view`), re-measured 3 seeds × 204,800 bd/arm/vul: **12/12 cells positive** — plain **+0.0078…+0.0125**, PD **+0.0073…+0.0111** IMPs/board, +1.4…+2.2/fired at ~0.5% firing. Details: [authored-reading-handoff.md](authored-reading-handoff.md#the-forensic--one-lane-one-bug). | fresh | **default-on 2026-08-16** (WIN/WIN) |
