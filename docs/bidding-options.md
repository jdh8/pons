...
| set_weak_two_major_priority | *(none)* | Natural | **ON** | over a weak **2♦ only**, a good five-card major outranks the Ogust ask — the ask sat at 2.0 above every new suit at 1.5, so a 6-5 in the majors with two diamonds asked about *diamond* quality. 2♥/2♠ is cheaper than 2NT and forcing, so Ogust is deferred, not lost; over 2♥/2♠ a new suit costs the three level and the ask keeps priority. A weight bump, not a gate on Ogust — `top_honors` is in the new-suit gate, so excluding "any five-card major" would orphan 14+ hands like ♠QJxxx ♦xx. **Enriched probe** (`--mode ogust`, same seed, 20 000 accepted of 15.0M draws, 0.069% density, 85% divergent): **PD +3.048/accepted, CI [+2.911, +3.184]; plain DD +1.668, CI [+1.563, +1.773]** — gain on both scorings, ≈+0.0021 IMPs/board scaled. Card unchanged (`Ogust = 1` is a boolean "we play it") | fresh | default-on ✓ (WIN/WIN). Open: 2NT's reading still does not deny a major. **Open, inherited 2026-08-18** from the closed [authored-reading campaign](authored-reading-handoff.md): the **`3♣` answer is unread**, so it projects as a natural ♣ `4..13` phantom suit. `bid_exclusion` cleared it off the rate-ranked worklist (`2♥/2♠/2♦ - 2NT - 3♣` at 95%/77%/86% partner-exclusion → 0) by folding the *strength* it declines, but the answer still publishes no club length of its own; a reader is the real fix and owes its own A/B |
...
| set_pdi_translate | `--ns-pdi-translate` | Natural | **OFF** | **A/B owed.** The PDI **dialect-translation shell** ([pdi.md](pdi.md)): where an authored rule carries the new `.pdi()` divergence tag, the auction handed to the distilled floor is rewritten into the picture its own teacher would have had (our tagged `X` → `P`; our sit over it → `X`), features come off that, and the legality mask plus the accountant gate stay on the real auction. Not the measured-loss P2 swap: the trigger is a per-rule tag on *confirmed-divergent* seats rather than penalty-ness, and the translation is input-side. One tagged rule today — §N1m's `X`@150, behind `--ns-landy-opener-px`, which BBA's book calls a **takeout double** with 2–4 of their major (2026-08-29 render). Inert in the default config by construction (nothing tagged is reachable); `smoke-default --count 20000 --seed 1` byte-identical | new | **hold** — measured by the `pxt` arm of `scripts/ab-landy-opener.sh` |
...
| `CompetitionKnobs::landy_pdi` | `--ns-landy-pdi`; `scripts/ab-landy-pdi.sh` | Artificial (`comp:landy-penalty`) | **OFF** | **A/B owed.** §N1n, the **P branch** of the Landy penalty process: after opener *passes* their advance (`1NT (2♣) X (2♥) -`) our later double of the major they run to is penalty on four-plus, not the takeout the floor reads. Two seats — responder over the overcaller's correction, opener over the advancer's run from `landy_doubler_px`'s delayed `X` (that shipped default-on rung's own interfered tail) — plus partner's sit under each. Rests on the **trigger theorem** ([pdi.md](pdi.md)): a pass/double inversion can only locate at a `P`, because hearing partner's `X` a double is illegal and there is no pair to invert. Carries no `.pdi()` tag on purpose — a divergent pass has no synonym in the floor's dialect, so this branch is repairable by authoring alone. Independent of `landy_opener_px` | new | **hold** — measured by `scripts/ab-landy-pdi.sh`, queued after §N1m's run |
