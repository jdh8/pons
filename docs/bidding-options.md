...
| set_meckstroth_minor_jumps | `ab-meckstroth-2nt --minor-jumps-only` | Natural | **OFF** (since 2026-09-26) | The Meckstroth adjunct's invitational `3m` jumps (5+ minor, 15–17), split out of `set_meckstroth_adjunct` 2026-07-26 so the leg could be priced alone (shipped book byte-identical). Published as **sd-vindicated** — plain wash, PD −0.0036/−0.0019, sd-lead **+0.0012/+0.0042**. **REFUTED 2026-07-26 by the isolated A/B** (400k/vul, 0.2% divergent vs the merged knob's 0.6% — the tell that the split took): plain DD −0.002/−0.003, PD −0.004/−0.005, plain SD −0.0007 ±0.0007 / −0.0003 ±0.0009 (wash, **not** the published win), **SD-PD −0.0025 ±0.0009 NV / −0.0021 ±0.0011 vul — CI-clear negative both**. Every bracket is ≤0; no bracket supports the ship. **Recommend demoting to opt-in** (confirm on a fresh seed first — 719/689 divergent boards). **CONFIRMED 2026-09-26 on a fresh seed** (400k/vul, SD seed 1790420932, 1057/1048 divergent): plain DD −0.002/−0.003, PD −0.005/−0.005, plain SD −0.0012 ±0.0007 / −0.0004 ±0.0009, **SD-PD −0.0029 ±0.0008 / −0.0021 ±0.0011**, CI-clear negative both again | **refuted (SD-PD), 2 seeds** | **demoted to opt-in 2026-09-26**. Note: `invitational_minor_continuations` still gates on the parent knob only, so responder's answers to a `3m` jump stay authored (reachable only if the floor jumps) — exactly the state both A/Bs measured |
| set_forcing_nt_two_suiter | `ab-forcing-nt-two-suiter` (no bba-gen flag) | Artificial | ON | Opener's invitational (15–17) major two-suiter over the forcing 1NT: `1♥ - 1NT - 2♠` reverse (5+♥ 4+♠) and `1♠ - 1NT - 3♥` jump (5-5 majors), both sides authored. plain wash-NV/**+0.001-vul** (never negative), PD −0.0017/−0.0010 (over-punished), **sd-lead +0.0012/+0.0013 NV, +0.0026/+0.0029 vul** (1M×2 seeds×2 vuls, SHA 293ed53; all four sd cells CI-clean; +0.8/+2.1 IMPs/div; fires 0.14%). sd redeems the PD loss — the `set_meckstroth_adjunct` profile. **REFUTED 2026-07-25 by SD-PD** (1M/vul, `sd-pd-readjudicate.sh`): the redemption *was* the missing doubling. plain DD wash both vuls (−0.000/+0.000), PD −0.002/−0.002, plain SD +0.0005 ±0.0004 / +0.0018 ±0.0006 (CI>0 both) → **SD-PD −0.0011 ±0.0005 NV (CI<0, sign flip) / +0.0000 ±0.0007 vul (dead wash)**. Every honest bracket is now ≤0 and no bracket supports the ship. The "`set_meckstroth_adjunct` profile" grouping is falsified with it — that knob *kept* 75-80% of its SD win under doubling (see its row), so "plain SD rescued it" was never a profile, just a coin flip between a real effect and an artifact that only SD-PD separates. **Recommend demoting to opt-in** (1388/1396 divergent boards, so confirm on a fresh seed before flipping). **NOT CONFIRMED 2026-09-26** (fresh seed, 1M/vul, SD seed 1790420932, 1347/1503 divergent): plain DD +0.000/+0.001, PD −0.001/−0.001, plain SD +0.0012 ±0.0004 / +0.0029 ±0.0006, **SD-PD +0.0000 ±0.0005 NV / +0.0012 ±0.0007 vul (CI>0)** — the two seeds pool to a wash. Repro: `ab-forcing-nt-two-suiter --sd` | **wash (SD-PD, 2 seeds)** | keep default-on; demotion withdrawn |
| set_forcing_nt_jump_shifts | `ab-meckstroth-2nt --jump-shifts` | Natural (+ `3NT!`, `4♣!`, the 5-5 alerts) | **OFF** | Natural strong jump shifts over the forcing 1NT, the rival to `set_meckstroth_adjunct` (inert while that is on): `2NT` = 18+ balanced uncapped, `3x` = natural 18+ jump shift (4+ cards, GF), `3NT!` = 18+ 6+M no side suit; displaces the two-suiter's `1♠ - 1NT - 3♥` leg (that 5-5 goes `2♥!` then a delayed `3♥!`). Built 2026-09-27. Draft 1 **lost** every bracket (400k/vul, SD-PD −0.0014 ±0.0011 NV / −0.0021 ±0.0014 vul); the trace (`--worst`) found responder-asks-RKCB, the 3NT-or-pass natural `2NT` tail and a floor-shadowing `Pass`. Repaired: **wash vs the shipped Meckstroth**, 2 seeds (SD seeds 1790452526 / 1790452759, 400k/vul): NV SD-PD +0.0011 ±0.0009 / +0.0006 ±0.0010, vul +0.0003 ±0.0012 / −0.0005 ±0.0012; plain DD and PD all within ±0.001. Fires 0.15% | **wash (SD-PD, 2 seeds)** | keep opt-in; Meckstroth stays default |
...
| set_weak_two_major_priority | *(none)* | Natural | **ON** | over a weak **2♦ only**, a good five-card major outranks the Ogust ask — the ask sat at 2.0 above every new suit at 1.5, so a 6-5 in the majors with two diamonds asked about *diamond* quality. 2♥/2♠ is cheaper than 2NT and forcing, so Ogust is deferred, not lost; over 2♥/2♠ a new suit costs the three level and the ask keeps priority. A weight bump, not a gate on Ogust — `top_honors` is in the new-suit gate, so excluding "any five-card major" would orphan 14+ hands like ♠QJxxx ♦xx. **Enriched probe** (`--mode ogust`, same seed, 20 000 accepted of 15.0M draws, 0.069% density, 85% divergent): **PD +3.048/accepted, CI [+2.911, +3.184]; plain DD +1.668, CI [+1.563, +1.773]** — gain on both scorings, ≈+0.0021 IMPs/board scaled. Card unchanged (`Ogust = 1` is a boolean "we play it") | fresh | default-on ✓ (WIN/WIN). Open: 2NT's reading still does not deny a major. **Closed 2026-08-24** (34445010, [reading-drift ledger](reading-drift-handoff.md#campaign-ledger)): the `3♣` answer used to project as a natural ♣ `4..13` phantom suit because its `hcp(0..)` fallback sibling made it shapeless and unalerted, so the walk read it as a new suit. `Alert("ogust")` on the ask and every answer lets the authored projection own the reading; it now reads 5–7 points, six in the major, no club length |
...
| `NotrumpKnobs::minor_transfer_slam_try` | `--ns-minor-transfer-slam-try off` (disable; default `13`); `PROBE_NT_MINOR_SLAM=off` | Natural + Artificial (`4m` + RKCB) | **`Some(13)`** | **Shipped default-on 2026-08-25.** Adds a natural `4m` slam try above each completed Puppet minor transfer, then opener's `4NT` RKCB/`5m` answer and the full keycard ladder. Two independent 8M-board rounds per arm/vul: `13` returned plain **+2.02/+2.66** then **+2.22/+2.91 IMPs/fired** NV/vul, with PD **+1.94/+2.59** then **+2.16/+2.85** and sd-lead positive throughout. The isolated 13–14 slice was positive in all eight plain/PD cells, pooling n=2662 at +0.26/+0.55 plain and +0.21/+0.50 PD IMPs/fired, so `13` took more total IMPs than `15` in every cell ([constructive rounds 1–2](minor-transfer-slam.md#constructive-round-1--measured-2026-08-25-seed-1787661918)) | fresh | default-on ✓; keep the explicit `off` arm for re-measurement |
| `NotrumpKnobs::minor_transfer_slam_fit` | `ab-nt-splinter --minor-slam 13 --minor-slam-fit`; `bba-gen --no-ns-minor-transfer-slam-fit` | Natural | **ON** | **Shipped default-on 2026-08-26.** After the `2NT` diamond transfer and opener's supported `3♦`, admits the exactly-5♦, 4+♣ member to the existing `4♦` slam try; the `3♣` denial stays 6+♦. Same-floor narrow-vs-wide, two fresh 8M-board seeds/vul (`1787678038`, `1787678117`), all eight DD/PD cells positive: plain **+0.99/+1.40 then +1.40/+1.88 IMPs/fired** NV/vul; PD **+1.00/+1.40 then +1.34/+1.83**, n=493/526 per cell and every conditional 95% CI above zero ([C1](minor-transfer-slam.md#c1-supported-54-extension--measured-2026-08-26--shipped)) | fresh | default-on ✓; explicit six-card control retained |
...
| set_pdi_latch | `--ns-pdi-latch` | Natural | **OFF** | **Action gate lost; the reading is authored and measured INERT.** Generalized pass/double inversion ([pdi.md](pdi.md)). The *action* gate — widened latch wrappers plus a configured-floor logit swap — lost on all four cells vs BBA at 204,800 bd/arm/vul (plain −0.0045/−0.0055, PD −0.0054/−0.0066, 0.22% fired, ≈−2.0 to −3.2 IMPs/fired) and was deleted: the v6 floor distils BBA, which already plays post-trigger methods, so the gate was a second inversion. What the knob now carries is reading-only — our post-trigger pass over RHO's live suit bid reads `[their-suit ≤ 4] ∪ [points ≤ 11]`, the negation of the trap, thresholds from `probe-pdi-population` (contradicts 0.85% of real passers, below the 0.974% ambient rate; holds out of sample). A **bid-only pre-count** (both arms bid, auctions diffed, no solver, `SEED_BASE=1787700673`) reads **10 divergent boards in 409,600 = 0.0024%**, so no DD time was spent; a reach-maximal control caps the mechanism at 0.032%. Cause: a union's hull is vacuous and `assemble` never recomputes `players` from the unions | new | **hold** — kept opt-in; the "goes live free if `players` is recomputed from the unions" hypothesis was **refuted 2026-08-26** by `set_union_hull` (0 call flips with PDI armed on both sides) |
| set_union_hull | `--ns-union-hull` | Natural | **OFF** | **Measured INERT — 0 call flips in 4.26M decisions.** `Inferences::assemble` recomputes `players[i]` from `unions[i].hull()` (narrow-only), closing the gap between what the book gates read and what the nets already read. Bid-only pre-count ([pdi.md](pdi.md), "The union-hull answer"; `scripts/precount-union-hull.sh`, `SEED_BASE=1787700673`, 204,800 bd/arm/vul vs BBA): a seat's hull moves on **50.9%/49.2%** of our decisions yet the arms are **byte-identical** — 0 of 819,200 tables. The drift is on `hcp`/`support_points` (the axes the nets gauge); `points` moves on 0.2% and `lengths` on 1.2%, which is why no argmax turns. Arming `pdi_latch` on both sides first still flips 0. | new | **hold** — kept opt-in; a candidate only behind a floor retrained on the tighter hulls |
...
| `CompetitionKnobs::multi_kokish_kraft` | `--no-ns-multi-kokish-kraft` (disable); `scripts/ab-2d-multi-kk.sh`; `PROBE_MULTI_KOKISH_KRAFT=0`, `probe-call-reading --no-ns-multi-kokish-kraft` | Artificial | **On** | **Shipped default-on 2026-08-25.** Whole-table swap of responder's `1NT (2♦)` Multi subtree for the Kokish–Kraft table: `X` at `hcp 8+` with no shape promise, a neutral pass with its own delayed *takeout* double, floorless `2NT`→♣ / `3♣`→♦ transfers, `3♠` both minors GF, a *penalty* repeated double, and the uncontested `4M` slam-try tier. Two departures from the design sketch, both recorded sub-arms: `3NT` **keeps** its both-majors stopper gate (bare it collapses the values double to `points 8..9`), and the `4M` tier inherits `direct_4m_max` = 15. Inert while their `2♦` is undeclared or natural, so the default system is byte-identical | `SEED_BASE 1787615025` | **SHIPPED DEFAULT-ON 2026-08-25** — re-measured on a fresh seed after the mirror book fixed the residue-1 leak (`SEED_BASE 1787615025`, SHA `f2ecb3c6`, 230 400 bd/arm/vul): **isolation gate 0 foreign at both vuls** (0/683 and 0/482, against a 55% prior rate), both-vul is the decision table's `win | win` row — plain **+0.0019 ±0.0013** / PD **+0.0023 ±0.0017** (+0.907/+1.102 per fired) — over a `wash | wash` NV (+0.0002 ±0.0012 / +0.0012 ±0.0015), sd-lead agreeing in all four cells and **no negative reading in eight** ([§N4-KK](archive/one-notrump-competitive-closed.md#n4-kk--the-kokishkraft-counter-a-whole-table-variant-shipped-default-on-2026-08-25)) |
| `CompetitionKnobs::multi_doubler_major` | `--no-ns-multi-doubler-major` (disable); `scripts/ab-2d-multi-doubler.sh`; `PROBE_MULTI_DOUBLER_MAJOR=0` | Natural | **On** | **Shipped default-on 2026-08-26.** Gives the Kokish–Kraft doubler a natural bid of the *other* major once their pass-or-correct resolves theirs — `2♠` over their `(2♥)`, `3♥` over their `(2♠)` — on `len(other, 4..)` at **weight 100**, below every existing rung, so it fires only on hands that would otherwise pass out the resolved partscore; plus opener's answer (game from the top of the range, the invitational raise where there is room, else pass) and responder's acceptance on `points 11+`. Two of four resolved paths carry it: `X (2♥) - (2♠)` is excluded because opener's pass denied four hearts, `X (2♠) - -` is withheld. Inert while their `2♦` is undeclared or natural, so the default system is byte-identical | `SEED_BASE 1787740671` | **SHIPPED DEFAULT-ON 2026-08-26 on jdh8's ruling, with one negative cell open.** 2.304M bd/arm/vul, **isolation gate 0 foreign at both vuls** (0/787, 0/510); 100% of divergences are "bid where the baseline passed", 0% the other way. Per fired — NV plain **+3.344** / PD **+0.610** (sd +4.412/+2.168); both-vul plain **+1.927** / PD **−1.737** (sd +3.399/+0.243). Both-vul PD is the decision table's `win | loss` row and sd rescues it only to a wash; the traced cause is opener's answer table having no notrump rung and no escape, so a maximum with a stopper and short support passes `2♠` in a 4-2 ([handoff](multi-doubler-answer-handoff.md)) |
| `CompetitionKnobs::multi_doubler_notrump` | `--no-ns-multi-doubler-notrump` (disable); `scripts/ab-2d-multi-doubler-nt.sh`; `PROBE_MULTI_DOUBLER_NOTRUMP=0` | Natural | **On** | **Shipped default-on 2026-08-27.** Opener's **notrump out** over the K–K doubler's natural other major: `3NT`@135 on `hcp(16..) & stopper_in(major)`, below the `4M`@140 game in a known 4-4 and above the `3♠`@130 invite, so it fires on exactly the hands `multi_doubler_major`'s answer table passes today. Built to repair that rung's **−1.737 IMPs/fired both-vul PD cell**, whose worst boards are `2♠` played in a 4-2/4-3 by a maximum with a stopper and short support ([handoff](multi-doubler-answer-handoff.md)). The same rung `multi_px_split` carries, unbundled so it prices against the shipped default; the two emit one rung. `smoke-default` byte-identical when off | `SEED_BASE 1787749549` | **SHIPPED DEFAULT-ON 2026-08-27 — a win in all four cells.** 4.608M bd/arm/vul (double the parent run: the surface is ~10× thinner, 1 in 43 000 at both-vul vs the parent's 1 in 4 500), **isolation gate 0 foreign at both vuls** (0/167, 0/106); 100% of divergences are "bid where the baseline passed" and 270 of 273 reach a game the baseline never bid. Per fired — NV plain **+2.910** / PD **+2.096** (sd +3.508/+2.933); both-vul plain **+4.264** / PD **+3.264** (sd +5.477/+4.729), every cell 3–4× its 0.417/0.523 SE. The hypothesis holds in direction — both-vul is the *larger* cell — but **not in magnitude**: +75 µIMP/bd against the parent's −384, i.e. ~20% of that deficit, so `multi_doubler_major`'s `win \| loss` row is improved, not retired ([handoff](multi-doubler-answer-handoff.md)) |
| `CompetitionKnobs::multi_doubler_minimum_notrump` | `--no-ns-multi-doubler-minimum-notrump` (disable); `scripts/ab-2d-multi-doubler-min-nt.sh`; `PROBE_MULTI_DOUBLER_MINIMUM_NOTRUMP=0` | Natural | **On** | **Shipped default-on 2026-08-27 — a win in all four cells.** Extends `multi_doubler_notrump`'s ladder down to the **15-count**: `2NT`@120 on `hcp(15..) & stopper_in(major)` below the `3♠`@130 invite, plus responder's `3NT`@140 acceptance on `hcp(10..)` (the 25 opposite a known 15). **One leg only** — `2NT` outranks spades at the same level so the rung exists over `2♠`; the `3♥` leg has no room below `3NT` and its 15-count keeps passing, which is why this was its own knob and its own seed rather than a one-point relaxation of the `hcp(16..)` floor. The hand it repairs is board 1 of the notrump out's worst tail (`A82.A53.KJ75.K74`, 15 HCP, three spades) ([handoff item 2](multi-doubler-answer-handoff.md)) | `SEED_BASE 1787769822` | **SHIPPED DEFAULT-ON 2026-08-27.** 4.608M bd/arm/vul, **isolation gate 0 foreign** at both vuls (0/144, 0/92), 100% of divergences "bid where the baseline passed". Per fired — NV plain **+2.007** / PD **+1.597** (sd +1.986/+1.816); both-vul plain **+2.011** / PD **+1.413** (sd +2.617/+2.160), against 0.449 and 0.562 SE, i.e. t = 4.5/3.6 and 3.6/2.5. The designed-for risk is real but paid for — 15 opposite `hcp 8+` is 23 combined and the worst boards are `2NT`/`3NT` going down, but 48% of divergences reach a game the baseline never bid, and on 8 boards the baseline's pass let the opponents balance into a game of *their* own that `2NT` shut out |
| `CompetitionKnobs::multi_minor_slam_try` | `--ns-multi-minor-slam-try off` (disable; default `15`); `PROBE_MULTI_MINOR_SLAM=off`; `scripts/ab-2d-multi-slam.sh` | Natural + Artificial (`4m` + RKCB) | **`Some(15)`** | **Shipped default-on 2026-08-25.** Completes each Kokish–Kraft minor transfer with the natural `4m` slam try, opener's `4NT` RKCB/`5m` answer and its keycard ladder; inert without the K–K counter and their declared `(2♦)` Multi. The fresh 10× run used 2.304M boards/arm/vul with 0 foreign divergences: `15` fired 95/40 times NV/vul for plain **+1.71/+3.67 IMPs/fired** and PD **+1.58/+2.78**; plain, PD and sd-lead were positive at both vulnerabilities. `15` fired 27% less often than `13` while taking more total IMPs, though the paired comparison did not separate the floors ([K–K Round 2](minor-transfer-slam.md#round-2--measured-2026-08-25-seed_base-1787642695--shipped-15)) | fresh | default-on ✓; `13` remains an explicit measured alternative and `off` the control |
| `CompetitionKnobs::landy_minor_slam_answer` | `--no-ns-landy-minor-slam-answer`; `PROBE_LANDY_SLAM_ANSWER=off`; `scripts/ab-landy-minor-slam.sh` | Natural + Artificial (`5m` / RKCB) | **`true`** | **Shipped default-on 2026-08-25.** Authors opener's previously missing answer above the N1j Landy `4m` slam try: `4NT` RKCB on `hcp(16..)`, otherwise `5m`, with the full keycard ladder. The 2.304M-board/arm/vul isolation gate was 0 foreign on all 27 divergences. Plain won **+5.56/+10.11 IMPs/fired** NV/vul (n=18/9), PD **+7.11/+11.44**, and sd-lead agreed in both cells; thin but unanimous, with one traced loss mechanism and a dead authored seat in the off arm ([Landy round 1](minor-transfer-slam.md#landy-round-1--measured-2026-08-25-seed-1787662790--win-thin--shipped)) | fresh | default-on ✓; retain the `--no-ns-*` control |
...
| --- | --- | --- | --- | --- | --- | --- |
| set_forcing_ceiling_read | `--ns-forcing-ceiling-read [true|false]` | Engine | **ON** | Phase 0b: a strong-1NT opener no longer treats a Lebensohl sign-off as forcing to game. Three seeds × 204,800 bd/arm/vul: **12/12 cells positive**, pooled +0.0001 plain / +0.0003 PD at both vulnerabilities, firing 0.01% at +2 to +6 IMPs each. The v6 nets were trained with this rail on; an off-arm now also prices that distribution shift. | fresh | **default-on 2026-08-16** (WIN/WIN) |
| set_strength_ceilings | `--ns-strength-ceilings [true|false]` | Engine | **ON** | Phase 1: made calls project both strength bounds instead of discarding the ceiling. Soundness gates improved; the pre-v6 raw arm exposed frozen-net OOD loss, while the compatibility arm moved 3/204,800 boards and won all four cells. Phase 5 retrained the policy and evaluator on the honest reading and removed the compatibility view. | fresh | **default-on 2026-08-16**; off is now v6-OOD |
| set_bid_exclusion | `--ns-bid-exclusion [true|false]` | Engine | **ON** | Phase 4 negative inference: a made bid excludes strictly heavier sibling gates it declined. Soundness partner exclusions **1.302%→1.180%**; three seeds × 204,800 bd/arm/vul were a wash on both scorers and vulnerabilities (93 divergences in 1,228,800, +36 plain / +51 PD pooled). | fresh | **default-on 2026-08-17**; off is now v6-OOD |
...
| set_reading_scope = `Alerted` | `--ns-reading-scope alerted` (`ab-alert-reading`) | Engine | opt-in (default until 2026-08-16) | reads alerted call as artificial (vs `None`, the whole-projection off arm): **plain +0.0171/+0.0234, PD +0.0207/+0.0278** NV/vul (1M×2, all CI>0, PD≥plain, +2.0-3.3 IMPs/div). Refreshes the stale-PD +2.08/+1.59; contested defense-switch value is extra. This became the historical v5 training view; `legacy_view` served it to the frozen nets until Phase 5 deleted the compatibility path | fresh | superseded by `All` as default ✓ |
| set_reading_scope = `All` | `--ns-reading-scope all` (default; `alerted` is the off arm) | Engine | **ON** | Projects **unalerted** (natural) authored calls too, without changing alerts or suppressing the walk. First whole-book run (seeds 1786883097/…532/…968) was **held**: plain +0.0014/+0.0018 but PD −0.0039/−0.0038 NV/vul. Forensic (`ab-dump-bucket --by lane`) put the entire loss in the four `1x (1NT)` lanes — the side-blind systems-on strip misreading partner's negative double as a 15+ penalty double of a 1NT opening. Strip fixed (ours-only; the then-frozen v5 nets held on their training view by the since-deleted `legacy_view`), re-measured 3 seeds × 204,800 bd/arm/vul: **12/12 cells positive** — plain **+0.0078…+0.0125**, PD **+0.0073…+0.0111** IMPs/board, +1.4…+2.2/fired at ~0.5% firing. Details: [authored-reading-handoff.md](authored-reading-handoff.md#the-forensic--one-lane-one-bug). | fresh | **default-on 2026-08-16** (WIN/WIN) |
...
`scripts/ab-landy-counter.sh` ([closed N1 history](archive/one-notrump-competitive-closed.md#n1--the-landy-2-counter-shipped-default-on-2026-08-14)).
