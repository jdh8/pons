...
| set_meckstroth_adjunct | `ab-meckstroth` (no bba-gen flag) | Artificial | ON | *(a misnomer — this is only opener's invitational `3m` jumps, NOT the real artificial-2NT adjunct; see `set_meckstroth_2nt`.)* plain wash (−0.001/+0.001), PD −0.0036/−0.0019 (over-punished), **sd-lead +0.0012/+0.0042** (2 seeds ×200k, SHA 22364c9; vul CI-clean +1.5–2.4/div, NV weakly +); sd redeems the PD loss — vindicates 21gf-ledger "keep". Repro: `ab-meckstroth --sd` | fresh | keep default-on ✓ |
| set_meckstroth_2nt | `ab-meckstroth-2nt` (no bba-gen flag) | Artificial | ON | The **real** Meckstroth adjunct: opener's `1M–1NT–2NT` = artificial 18+ GF (any shape) + `3♣`-relay shape-outs, both sides + RKCB. plain **+0.0075/+0.013**, PD +0.006/+0.011, **sd-lead +0.010/+0.017** NV/vul (200k×2 seeds, all CI-clean; +2.7/+4.4 IMPs/div; fires 0.4%). Repro: `ab-meckstroth-2nt --sd` | fresh | keep default-on ✓ |
