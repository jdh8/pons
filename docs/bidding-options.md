...
| `CompetitionKnobs::multi_kokish_kraft` | `--ns-multi-kokish-kraft`; `scripts/ab-2d-multi-kk.sh`; `PROBE_MULTI_KOKISH_KRAFT`, `probe-call-reading --ns-multi-kokish-kraft` | Artificial | **Off** | **Built 2026-08-25, unmeasured.** Whole-table swap of responder's `1NT (2♦)` Multi subtree for the Kokish–Kraft table: `X` at `hcp 8+` with no shape promise, a neutral pass with its own delayed *takeout* double, floorless `2NT`→♣ / `3♣`→♦ transfers, `3♠` both minors GF, a *penalty* repeated double, and the uncontested `4M` slam-try tier. Two departures from the design sketch, both recorded sub-arms: `3NT` **keeps** its both-majors stopper gate (bare it collapses the values double to `points 8..9`), and the `4M` tier inherits `direct_4m_max` = 15. Inert while their `2♦` is undeclared or natural | owed | **A/B owed** — two arms vs shipped v7, both vuls, plain + PD ([§N4-KK](one-notrump-competitive.md#n4-kk--the-kokishkraft-counter-as-an-opt-in-whole-table-variant-built-2026-08-25-ab-owed)) |
