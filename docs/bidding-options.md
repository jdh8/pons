...
| set_dnf_reading | `--ns-dnf` (`scripts/dnf-flip-ab.sh`) + `ab-dnf-sd-lead` | Engine | OFF | DNF union-of-boxes reading ([dnf-migration.md](dnf-migration.md)): disjunctions keep separate boxes instead of hulling to ⊤. **Flip REFUTED as-is — LOSS all 4 cells** (204800/arm/vul, SEED 1784784503): plain −0.0038/−0.0054, PD −0.0039/−0.0053 NV/vul, all CI<0; fired ~1%, −0.37/−0.60 per fired; worst tail = under-reach (stops below making slams — feature-shift hypothesis: the frozen net is calibrated to hulled features). sd-lead matrix (20k×2, seed 1784779888): NV −0.0318 ±0.0267, vul +0.0121 ±0.0315, pooled wash | fresh | keep default-off opt-in; flip path = regen+retrain with knob-on features (user checkpoint) |
