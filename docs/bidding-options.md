...
| set_diamond_splinter | `ab-nt-splinter --diamond` (no flag) | Artificial | **ON** | responder's `3♥`/`3♠` after the `2NT` diamond transfer — a void or low singleton with game values, so opener places the game (`3NT` with the short suit stopped, else `5♦`) instead of responder guessing `3NT`. Mirrors the club side's long-shipped lane; offered only where the fit is assured (over `3♦` opener's three-card support covers the whole `2NT` class including 5♦4♣; over the `3♣` denial it needs a self-sufficient 6+♦). 6M/vul, seed 1786613240, 5103 fired (0.085%) / 650 divergent: **plain +1.01/+1.23 IMPs/fired** (+0.0009/+0.0010 per board) ÷ **PD +1.20/+1.44** NV/vul — positive on both scorings, no doubling artifact; sd-lead agrees (+0.0007/+0.0008 plain, +0.0008/+0.0010 PD per board, 95% CI ±0.0001). Self-play: BBA has no counterpart toggle, so a measurement against it would price misinformation. Uses `splinter_short` (excludes a stiff A/K as a working honor) where the club twin uses bare `len(short, ..2)` — **open**: aligning them is its own A/B. European keeps the old binary table; its `3♦` is a blind completion, not a fit promise | fresh | default-on ✓ (WIN/WIN; knob = off-arm) |
