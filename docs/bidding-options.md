...
| set_queen_ask | — | Artificial | **OFF** | **Authored 2026-08-01, unmeasured.** Adds the queen relay to both keycard ladders: after a one-or-four or none-or-three answer — the counts that leave the trump queen open — the asker relays one step and partner replies on the next two rungs — or jumps to **six of trumps**, the third answer: no queen, but a buff the ladder has no rung for (a ninth trump or a side-suit void, `set_queen_buff_fit`, default 9), bid because a denial there gets passed in five and six makes 76.0% DD / 82.9% de-biased. A king ask sits above the queen-shown reply. Decision table: five keycards bid six whatever the queen does; four keycards need the queen; a denial stops at five; where the relay has no room (plain 4NT in a minor, or hearts after a none-or-three) the asker bets six on four exactly as before. A **proven ten-card fit** stands in for the queen on both sides (BBA's `posiadane_karty >= 10`, `set_queen_fit`) — ten and not nine, because that rung has to hold up for a grand and nine runs the suit only 53.1% of the time. The asker's threshold is the shorter one: with a known nine-card fit the reply cannot change the call, so it bids six without asking. The king ask rides the grand-zone strength gate, not the keycard count — RKCB is a slam veto, not a slam seeker — and seven is bid on two of the three side kings. Geometry is one `bid_successor` walk shared by book and floor, so the two ladders cannot drift; it fits exactly when the *no-queen* rung still lands at or below five of trump. **Read at build time by both `instinct()` and the book's `install_rkcb`**, plus the recognizers at classify time — build one stance per arm *and* set the flag per call by side. Fires ~10.7 boards per 10⁴ (30k smoke), dense enough for a random-deal A/B; expect a grand-heavy divergent set and apply the slam-boundary shave | — | opt-in, pending `ab-kickback --feature queen --baseline gated` |
