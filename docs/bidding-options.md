...
| set_dnf_reading | `--ns-dnf` (bba diffpair) + `ab-dnf-sd-lead` | Engine | OFF | DNF union-of-boxes reading ([dnf-migration.md](dnf-migration.md)): disjunctions keep separate boxes instead of hulling to ⊤. sd-lead knob matrix (20k×2, seed 1784779888): NV **−0.0318 ±0.0267**, vul +0.0121 ±0.0315 — pooled wash, NV cell a caution. Chop-F flip verdict = the full bba match, in flight | fresh (sd only) | pending chop F |
| set_gauge_membership | `--ns-gauge-membership` + `ab-dnf-sd-lead` | Engine | OFF | strength gauges get membership teeth (samplers reject outside raw-HCP/support-points bands — the one knob that can reject legal hands; eval⟹membership sweep is its soundness gate): **WASH every cell** (20k×2 — gauge vs off −0.0034/−0.0042; on top of dnf +0.0140/+0.0002, all CI⊇0) | fresh | keep default-off kill-switch; ride-along decided with chop F |
