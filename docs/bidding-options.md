...
| set_opener_third | `--no-ns-opener-third` | Natural | ON | opener's third call after trump is agreed at `1M–2r–R–3M`: 4NT RKCB on `points(15..)`, else an unconditional `4M`, every cue-bid and five-level call at −∞. **Re-audit candidate #2 — deletion measures positive and is REJECTED anyway:** +0.437/+0.527 plain, +0.524/+0.637 PD IMPs/div NV/vul (self-play 2,000,000×2, seed 1784484826, 971 div = 0.05%), but with the node gone the floor never asks keycards here at all — it signs off 4♠ on a **26-count** opposite a game force, stranding every slam. +0.0003 IMPs/board does not buy a total capability loss. Resumable via the `set_two_over_one_force` pattern: delete the node *and* give the floor a controls-based ask | fresh | keep default-on (deletion blocked on floor slam capability) |
