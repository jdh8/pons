...
| `CompetitionKnobs::multi_doubler_notrump` | `--ns-multi-doubler-notrump` (enable); `scripts/ab-2d-multi-doubler-nt.sh`; `PROBE_MULTI_DOUBLER_NOTRUMP=1` | Natural | **Off** | Opener's **notrump out** over the K–K doubler's natural other major: `3NT`@135 on `hcp(16..) & stopper_in(major)`, below the `4M`@140 game in a known 4-4 and above the `3♠`@130 invite, so it fires on exactly the hands `multi_doubler_major`'s answer table passes today. Built to repair that rung's **−1.737 IMPs/fired both-vul PD cell**, whose worst boards are `2♠` played in a 4-2/4-3 by a maximum with a stopper and short support ([handoff](multi-doubler-answer-handoff.md)). The same rung `multi_px_split` carries, unbundled so it prices against the shipped default; the two emit one rung. `smoke-default` byte-identical | `SEED_BASE 1787749549` | **A/B IN FLIGHT 2026-08-26**, `ab-results/2d-multi-doubler-nt/`, 4.608M bd/arm/vul — double the parent run, because a 120k smoke reads the surface at ~1 in 60 000 (vs the parent's 1 in 4 500), 100% "bid where the baseline passed", 0 foreign |
