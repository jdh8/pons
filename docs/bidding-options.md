...
| set_pdi_latch | `--ns-pdi-latch` | Natural | **OFF** | **A/B owed.** Generalized pass/double inversion ([pdi.md](pdi.md)): any penalty-oriented double we made (rule tag) or any pass of ours that left partner's double in makes our later doubles read and bid as penalty. Shipped opt-in 2026-08-26, mechanism byte-identical at the default. Dominant firing lane is the structural takeout-X conversion; K–K is the zero-delta control | new | measure (P2) |
