...
| set_kickback | `--no-ns-kickback` | Artificial | **ON** | **Shipped default-on 2026-08-02** at jdh8's call, on a PD win against a vulnerable plain-DD loss — not a decision-table pass, and recorded as a judgement in `docs/ai-bidder/bba-kickback.md` §7.10. The clean cell (2×10M boards, seed 1785623878, after §7.6's collision guard and with the floor's kickback twin selected by the same knob): **NV +0.0723 PD / +0.0062 plain DD, vul +0.0438 PD / −0.0078 plain DD** per board. It prices the *package*, not the relocation — 95% of the divergent boards never reach the six-level, so the twin net dominates the aggregate (§7.8). **The per-trump split by contract strain is RETRACTED (§7.9)** — contract strain does not identify the lane an ask was made in, so it sliced the net's rewrite by strain. Bucketing by the *ask* instead: a relocated ask fires on **35 of 63,203 divergent boards (0.055%)**, moving −16 PD / −23 plain DD in a cell that moved +13,522 / −850, while **93.7% of divergent boards saw no keycard ask at all** and carry 89% of the PD gain. Corroborated by the phase-6 cell (216 divergent boards per 1M against `gated`). Kickback is near-inert at random-deal density; a rare-trigger convention wants enriched probing, not more random boards. The earlier −0.0022/board (1M, 2026-08-01) is **contaminated and must not be cited**: it predates both the face gate and the collision guard. Relocates the keycard ask onto the face-only `kickback_ladder`: 4♦ asks in clubs and 4♥ in diamonds (*Redwood*), 4♠ in hearts (*Kickback* proper), so every 1430 answer lands at or below five of trump. 4NT keeps its own meaning — kickback *adds* asks, never removes one. **Read at `instinct()` build time as well as classify time**: the reading's `alerted` test is structural, so the relocated rules must be absent (not merely inert) in the off arm. Build one stance per arm *and* set the flag per call by side. The knob also selects the floor's weights (`classify_bba` serves the kickback twin), and the convention card discloses it (`Kickback 1430 = 1`) | 2×10M | shipped default-on |
