...
Directly relevant to `evaluator.rs` and `accountant_floor`:
...
| D | **Expected-score level choice** | **Constructive rung DONE** (2026-07-21): the floor's game/slam boundary gates (combined-25/fit-sum-31 games, 33/37 slams, the RKCB entry, the asker's grand) price `P(make)` via `Gaussian::p_at_least` against the IMP break-evens (our failing branch priced *doubled* — a gate prices a call, and bid = PD is the scoring split), vulnerability-aware, behind `accountant_floor` (**default on** — `examples/ab-accountant-floor` measured it at both vulnerabilities, both scorers winning, at v1 and again at the v2 weights; see [evaluator-net.md](evaluator-net.md#against-the-shipped-floor)). Still open in-row: the full `Σ P(T=k)·score(k)` integration, doubled contracts, and a probabilistic par over all 20 declarer columns for the *competitive* decisions (settle rail, sacrifices) — the natural follow-up session. **Designed 2026-08-12**: [competitive-accountant.md](competitive-accountant.md) (the contested 5-level gate, leaf form; full par chartered as its v2) + [doubling-calibration.md](doubling-calibration.md) (the P(double) evidence and q table). | `src/bidding/instinct.rs` |
| G | **Envelope-gated new-suit veto** | **MEASURED AND REFUTED 2026-09-02 — opt-in, default off.** Plain DD −0.0212 ±0.0047 (none) / −0.0164 ±0.0057 (both), PD a wash, 204,800 bd/arm/vul, seed 1788352713. The shell's second demotion-only stage: after the legality mask, `new_suit_gate` sets `-∞` on every suit bid in a suit where we hold at most four cards and our length plus partner's *announced* minimum reaches at most five — a trump suit nobody promised. The net cannot fix this itself: `features_v6` sets a we-bid-this-strain bit for artificial calls and has no alert column, so a cue and a natural suit bid are the same input and repairing that is a retrain. Origin: the 2026-08-14 `1NT (2♣)` residue; evidence: the §N1-lia lia3 forensic, where the *default system's own* floor made such calls on 6,800/3,844 boards at 5.2/6.5 IMPs per fired. The forensic is the durable part: not narrowable by level (every level negative, the four-level class *least* costly per fired — refuting the pool that motivated it), the whole loss is `own == 4`, and partner's announced minimum was 0 on 98% of fires, so the envelope gate did no work and the rail was really "never name a suit you hold four or fewer of". An announced minimum of zero means *unspoken*, not *short* — a successor needs the negative-inference channel, i.e. an input-side retrain. Design [new-suit-veto.md](new-suit-veto.md). | `src/bidding/instinct.rs`, `src/bidding/neural_floor.rs` |
...
disaster tails genuinely matter (designed 2026-08-12; see row D's links). Note what C did *not* need: decoding the
