...
| Distillation | Train a fast "student" to copy a "teacher"'s output distribution. Here: student net copies `american_instinct()`'s softmax (the deterministic pair — never the net-floored `american()`, which would be circular). |
