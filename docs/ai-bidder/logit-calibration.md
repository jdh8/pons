...
| 8 (2026-09-16) | **cache the solves, not the swings**: `dump-teacher --relabel` writes a `.dd` sibling (per decision, every solved layout as a `.pdd` row); a chunk priced at another `git_sha` is re-priced from its tables instead of skipped or extended (its `.ret` is trusted only at the commit that wrote it); `--diff A B`, the drift census of two dumps of one window; the worker's skip gate reads the sidecar's commit. M64 **declined for now**: its label value over M32 (+0.73 vs +0.62 DD, +0.52 vs +0.46 PD per relabelled decision, fire 17.2 vs 13.8%) is inside §4d's own intervals and prices at roughly +0.02 PD / DD wash shipped, for another fleet-week at the frozen corpus SHA | byte-identity: **no `src/` edit at all**; the example's five relabel tests pass, including a re-price at a foreign SHA that solves 0 layouts and reproduces the `.ret` byte for byte, then extends to match a native draw | **done** — the 188 fleet chunks predate the cache, so their first re-price pays the 64 layouts once; every pass after is bidding-bound. Recipe in [../shared-machine-data-gen.md](../shared-machine-data-gen.md#re-pricing-a-chunk-and-the-drift-census-2026-09-16) |
