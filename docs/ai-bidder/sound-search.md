...
- **Seam:** [ev.rs:132-141](../../src/bidding/ev.rs) — the scoring line `ns_score_bid(result, tricks, vul)` at [ev.rs:138](../../src/bidding/ev.rs). The swap needs (a) leader-view **and** declarer-view inferences per reached contract, computed with `Partnership::infer` on the rolled-out prefix — the exact idiom already in [examples/common/mod.rs:213-249](../../examples/common/mod.rs) (`sd_declarer_ns_score`) — but `ev_all` is generic over `Bidder` while `infer` lives on `Partnership` ([book.rs](../../src/bidding/book.rs)), so it needs `ev_all` narrowed to `&Partnership` or a new `Bidder::infer`, not a bare scoring-line swap; (b) a perfect-defense-doubling variant of `ns_score_tricks` ([scoring.rs:190](../../src/scoring.rs)) — double when `sd_tricks < 6 + level`, so a failing sacrifice still prices doubled (do **not** copy the example's plain `ns_score_tricks`, which drops the doubling ev_all's semantics require).
...
1. **Phase 2** — the `SD_EVAL` offline scorer (the highest-value *correctness*
   win; the scoring swap at [ev.rs:138](../../src/bidding/ev.rs) plus two-view inference plumbing — mirrors `sd_declarer_ns_score`, but `ev_all` must first gain `Partnership` access, being generic over `Bidder` today).
