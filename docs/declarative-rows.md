...
| Phase 2 (cross-side assembly) | **Both halves built and REFUTED** 2026-08-07, both default off. 2a — the net's card channel (`--declare-opponents`). A follow-up run the same day priced a **one-bit off-manifold** `theirs` perturbation at −0.01 plain / −0.02 PD per board; it was *meant* to be the in-distribution cell-B test and is not one. **Cell B at scale ran 2026-08-08** (204 800 bd/arm/vul: plain wash, PD positive, +0.0062 ±0.0048 vul CI-clear — see §2a below); the v5 corpus-spend verdict lives in [card-manifold.md](ai-bidder/card-manifold.md). 2b — the reader's (`Partnership::with_opponents`, `--declare-their-book`). See below. The book-selection third leg (`defense_vs`, `competitive_vs`, `Table::compose`) is **not built and not wanted**: `defensive_vs` already existed, had zero production consumers, and went out with `Family`. |
