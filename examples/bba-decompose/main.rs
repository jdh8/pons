...
enum OurFloor {
    ...
    American,
    /// The v6 twin retrained on BBA's disclosed Multi-Landy readings
    AmericanV6Their,
...
impl OurFloor {
    fn partnership(self) -> Partnership {
        ...
        match self {
            ...
            Self::AmericanV6Their => american_v6_their(&agreements),
...
struct Args {
    ...
    /// anchor unchanged; current BEN dumps require `american` (and the derived
    /// `--european-minors` opponent book).
    #[arg(long, value_enum, default_value_t = OurFloor::AmericanInstinct)]
    our_floor: OurFloor,

    /// Replay with BEN's European minor-transfer reading declared as the
    /// opponent book, as `ben-gen` has bound it since 2026-08-13.  Unset,
    /// derived from the dump's opponent label (BEN → on); pass `false` to
    /// replay BEN dumps generated before that date (the 2026-08-10 Tier-F
    /// corpus).
    #[arg(long, num_args = 0..=1, default_missing_value = "true")]
    european_minors: Option<bool>,
    ...
    /// Replay with the disclosed-Multi union reader
    /// (`bba-gen --ns-their-multi-read`).  Unset tracks the engine default
    /// (on since the 2026-08-16 N4 residue ship).
    #[arg(long, num_args = 0..=1, default_missing_value = "true")]
    ns_their_multi_read: Option<bool>,

    /// Replay with the Multi advance suppression used by the BBA-reading twin.
    #[arg(long)]
    ns_their_multi_advance_read: bool,

    /// Replay with the values-double floor used by the BBA-reading twin.
    #[arg(long)]
    ns_their_multi_double_read: bool,

    /// Replay the Multi `3♠` stopper-ask arm generated by
    /// `bba-gen --ns-multi-stopper-ask`.
...
fn main() -> anyhow::Result<()> {
    ...
    let partnership = if landy {
        ...
        agreements.competition.multi_stopper_ask = match args.ns_multi_stopper_ask.as_str() {
            ...
            other => {
                anyhow::bail!("--ns-multi-stopper-ask must be off|search|place, got {other:?}")
            }
        };
        ...
        match args.our_floor {
            ...
            OurFloor::AmericanV6Their => american_v6_their(&agreements),
    ...
    // `ben-gen` declares BEN's European minor scheme as the opponent book (one
    // axis only; see its comment).  Reading their `1NT - 2♠`/`3♣` as our Puppet
    // relay instead moves our own later calls, so the replay gate needs the
    // same binding.
    let european = args
        .european_minors
        .unwrap_or_else(|| arms.iter().any(|arm| arm.their_label.contains("BEN")));
    let partnership = if european {
        let mut theirs = pons::bidding::agreements::Agreements::default();
        theirs.decision.reading.notrump_minors = pons::bidding::american::EUROPEAN;
        partnership.with_opponents(&american(&theirs).bind())
    } else {
        partnership
    };
