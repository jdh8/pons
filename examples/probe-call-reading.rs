...
struct Args {
    ...
    /// Disarm §N1q's strength-sorted two-level majors
    /// (`competition.defense_2c_landy_strength_majors`, default on): `2♠` = 4+♣ 4+♦
...
fn main() {
    ...
    if args.ns_landy_lia && !args.their_2c_landy {
        eprintln!(
            "warning: --ns-landy-lia is inert without --their-2c-landy; this run probes the systems-on default lane"
        );
    }
