//! Throwaway: dump the **shipped Dutch** system's auctions on seeded deals.
//!
//! Built at two commits and diffed, this answers exactly one question — did a
//! refactor move a bid in the Dutch system?  Nothing here is measurement; it
//! is a byte-identity check, so it takes no knobs and no arms on purpose.
//!
//! ```text
//! cargo run --release --example smoke-dutch -- --count 20000 --seed 1
//! ```
