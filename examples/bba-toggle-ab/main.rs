...
// Addressing recovered from objdump and validated against 21GF.bbsa (240/258
// boolean toggles round-trip via get_conventions); lets us flip ONE named
// convention to isolate it in a BBA-vs-BBA A/B.
...
struct BbaOracle {
    ...
    /// the single-toggle lever: load both sides at the same `system`, override
    /// one convention on one side, and the match isolates it.
...
impl System for BbaOracle {
    fn classify(&self, hand: Hand, vul: RelativeVulnerability, auction: &[Call]) -> Option<Logits> {
        ...
        let code = unsafe {
            ...
            // Force the isolated convention(s) AFTER set_system loads defaults.
...
fn main() -> anyhow::Result<()> {
    ...
    // Single-toggle A/B: both sides are the SAME BBA system; the only difference
    // is the named convention, forced ON for our pair and OFF for theirs.  Every
    // divergent board is then a pure effect of that one toggle.
    let toggle = std::env::var("TOGGLE").unwrap_or_else(|_| "Rubensohl after 1m".into());
    let name = CString::new(toggle.clone()).expect("convention name has no NUL");
    let load = |value: c_int| BbaOracle::load(&path, args.system, vec![(name.clone(), value)]);
    let bba = match load(0) {
        Ok(bba) => bba,
        Err(error) => {
            eprintln!(
    ...
    let our_oracle = load(1)?;
    let ours: &dyn System = &our_oracle;
    let our_label = format!("BBA {} [{toggle} ON]", system_label(args.system));
    let mut rng = rand::rng();
    let _ = american; // floor unused in this toggle-isolating fork
    let _ = Family::NATURAL;
    ...
    println!(
        "=== {} (us) vs BBA {} [{toggle} OFF] (them): {} boards, vulnerability {} ===",
        our_label,
