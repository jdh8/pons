...
//! two-suiter rebids.  Each board is bid twice over the same deal, once per arm;
//! boards whose arms reach different contracts are solved double dummy once and
//! scored.  A positive IMPs/board favors the treatment.
//!
//! ```text
//! cargo run --release --example ab-forcing-nt-two-suiter -- --count 5000
//! ```
...
/// Forcing-1NT two-suiter A/B: minimum natural rebid vs invitational reverse/jump
#[derive(Parser)]
struct Args {
...
fn main() {
    ...
    // arm 0 = baseline (two-suiter off), arm 1 = treatment (two-suiter on, the
    ...
    println!(
        "=== Forcing-1NT two-suiter A/B: {} boards, vulnerability {} ===",
    ...
    println!(
        "Two-suiter: {points:+} points, {total_imps:+} IMPs ({:+.3} IMPs/board plain)",
    ...
    if args.sd {
        ...
        println!(
            "sd-lead two-suiter ({} worlds, seed {}): {total:+} IMPs, {mean:+.4} IMPs/board [95% CI ±{ci:.4}], {:+.3} IMPs/divergent",
            args.sd_worlds,
            args.sd_seed,
