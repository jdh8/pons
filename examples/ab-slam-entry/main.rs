...
//! play the same books; the knob is pinned into a partnership at build, so each side
//! bids off its own pre-built partnership.  Divergent boards are scored two ways from
//! the same DD table:
...
///
/// The gate is pinned into a partnership at build, so the two sides bid off two
        ...
        for &index in &divergent {
            ...
            let [a, b] = [&board.table_a, &board.table_b].map(|auction| {
                common::sd_declarer_ns_score(
                    ...
                    // `ReadingProfile` — so either partnership reads identically.
                    &partnerships[0],
                    args.vulnerability,
                    &mut rng,
                    args.sd_worlds,
