...
struct Args {
    ...
    /// Turn OFF the rail that vetoes our silent side's double over their `2NT`
    /// opening auction (`InstinctProfile::their_2nt_double_veto`, crate
    /// default on).  The `plain` arm of `scripts/ab-2nt-double-veto.sh`.
    #[arg(long, default_value_t = false)]
    no_ns_2nt_double_veto: bool,

    /// Turn OFF the rail that vetoes our silent side's `4NT` over their `3NT`
    /// without two five-card suits (`InstinctProfile::their_3nt_unusual_veto`,
    /// crate default on).  The `plain` arm of `scripts/ab-3nt-unusual-veto.sh`.
    #[arg(long, default_value_t = false)]
    no_ns_3nt_unusual_veto: bool,

    /// Turn OFF the rail that vetoes our silent side's suit pull of their game
    /// into a suit of four cards or fewer (`InstinctProfile::their_game_pull_veto`,
    /// crate default on).  The `plain` arm of `scripts/ab-game-pull-veto.sh`.
    #[arg(long, default_value_t = false)]
    ...
    /// Turn OFF the rail that vetoes our silent side's bids over their `2NT`
    /// opening auction on 7 HCP or fewer (`InstinctProfile::their_2nt_bid_veto`,
    /// crate default on).  The `plain` arm of `scripts/ab-2nt-bid-veto.sh`.
    #[arg(long, default_value_t = false)]
    no_ns_2nt_bid_veto: bool,
    ...
    /// major (`1NT (2♦) X (2♥) - - 2♠ -`)
    ///
    /// `3NT`@135 on `hcp(16..) & stopper_in(major)`, below the `4M`@140 game
    /// in a known 4-4 and above the `3♠`@130 invite, so it fires on exactly
    /// the hands that pass today — which is where the shipped
    ...
    /// redefined in place 2026-09-01, then **refined on the lia2 forensic**
    /// after that build measured a loss: `2♠`/`2NT` are six-card minors
    /// invitational-or-better answered by super-accept / `3NT` / completion,
    /// the values `X` narrows to two-plus in each major, `2♥` is the one
    /// both-minors takeout in two bands *below* it, and `3♦`@142 / `3♣`@141
    /// are six-card sign-offs over a `2♦` escape whose ceiling rises to eight
    /// HCP.  A permutation — one arm, it cannot be decomposed.
    #[arg(long, default_value_t = false)]
    ns_landy_lia: bool,
