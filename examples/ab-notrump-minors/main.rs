...
//! baseline (PD-era), but Puppet-vs-European was never isolated — this harness does
//! that.  The choice is read once at book-construction time (`gated` is eager), so
//! each arm bakes its own minor table and the tries are independent thereafter.
//!
//! Opponents are silenced (East/West always pass), so every auction is
//! constructive start to finish — this measures the *constructive* value of Puppet
//! over European (which minor scheme lands the better contract), not any
//! competitive or lead-directing value.  Each board is bid twice over the same
//! deal, once per arm; boards whose arms reach different contracts are solved
//! double dummy once and scored.  A positive IMPs/board favors Puppet (the default).
//!
//! ```text
//! cargo run --release --example ab-notrump-minors -- --count 5000
//! ```
...
fn main() {
    ...
    println!(
        "=== 1NT minor-scheme A/B (Puppet vs European): {} boards, vulnerability {} ===",
    ...
    println!(
        "Puppet (vs European): {points:+} points, {total_imps:+} IMPs ({:+.3} IMPs/board plain)",
    ...
    if args.sd {
        ...
        println!(
            "sd-lead Puppet ({} worlds, seed {}): {total:+} IMPs, {mean:+.4} IMPs/board [95% CI ±{ci:.4}], {:+.3} IMPs/divergent",
            args.sd_worlds,
            args.sd_seed,
