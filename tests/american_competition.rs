...
/// three cards in each of their *named* suit — so an opener without four of
/// the major they advanced never reaches the seat this ladder answers.  The net floor the A/B measures passes there
/// (`probe-decision` reads `P` 11.9 against `X` 3.0), which is the traffic the
/// rung is authored for.
...
/// Walk a whole `1NT (2♣)` auction on the Kokish–Kraft variant of the Landy
/// counter (`competition.defense_2c_landy_kk`)
fn walk_landy_kk(opener: &str, responder: &str, theirs: &[Call], expected: &[Call]) {
    let mut agreements = pons::bidding::agreements::Agreements::default();
    agreements.decision.their.two_clubs_landy = true;
    agreements.competition.defense_2c_landy_kk = true;
    let system = american(&agreements).bind();
...
#[test]
fn landy_kk_cross_transfers_the_club_suit_and_stops_in_game() {
    // 1NT (2♣) 3♦ - 3♥ - 3NT: `3♦` is *clubs*, so the six-card suit is hidden
    // and the `1NT` declares.  Opener has only one major guarded, shows it
    // below game rather than completing to `4♣`, and responder — holding the
    // other — bids the notrump game from the strong side.  None of this
    // auction exists on the shipped ladder, where `3♦` is not a call at all.
    walk_landy_kk(
        "952.KJ9.AQ432.T5", // 15, hearts guarded, spades not
        "AK4.83.5.KQT8543", // game-forcing with a solid six-card club suit
        &[call(2, Strain::Clubs), Call::Pass, Call::Pass, Call::Pass],
        &[
            call(3, Strain::Diamonds), // responder: clubs, game-forcing
            call(3, Strain::Hearts),   // opener: hearts guarded, spades not
            call(3, Strain::Notrump),  // responder: holds spades, bids the game
        ],
    );
}

#[test]
fn landy_kk_escape_relay_lands_the_weak_six_card_minor() {
    // 1NT (2♣) 2NT - 3♣ - 3♦: the escape the shipped ladder cannot make, its
    // `2NT` being a club transfer.  Responder has nothing and a six-card
    // diamond suit; opener is forced to `3♣` and responder corrects.  Opener
    // then sits — the band tops out at seven high-card points.
    walk_landy_kk(
        "AQ5.KJ9.A43.T543", // a flat minimum with no reason to move
        "543.J8.KT8543.Q5", // six diamonds, six high-card points
        &[call(2, Strain::Clubs), Call::Pass, Call::Pass, Call::Pass],
        &[
            call(2, Strain::Notrump),  // responder: a weak six-card minor
            call(3, Strain::Clubs),    // opener: the forced relay
            call(3, Strain::Diamonds), // responder: corrects to the real suit
            Call::Pass,                // opener: sits
        ],
    );
}
