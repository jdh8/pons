...
/// [`advance_of_double_package`] (`P* (2X) X -`) and
...
pub(super) fn sohl_rows_over(
    ...
    if style == LebensohlStyle::Transfer && over != Suit::Diamonds {
        ...
        for bid_suit in [Suit::Clubs, Suit::Diamonds, Suit::Hearts, Suit::Spades] {
            ...
            entries.extend(rows_of(Pattern::node(&format!("{base} {resp} -")), reply));
        }
        // Delayed cue: `base 2NT - 3♣ - 3X` (their suit) — Stayman with a stopper,
        ...
        if recognize {
            ...
            entries.extend(rows_of(
                Pattern::node(&format!("{relay} 3♣ - {cue} -")),
                cue_stayman_answer(over),
            ));
        }
    }
