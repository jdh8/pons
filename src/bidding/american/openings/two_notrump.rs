...
pub(super) fn with_two_notrump(rules: Rules, agreements: &Agreements) -> Rules {
    ...
    //
    // The cell is read at classify time too (`inference/readers.rs` caps the
    // 2NT opener's minor length on it), so it lives in `DecisionProfile` and is
    // read from there here rather than duplicated into `OpeningKnobs` — one
    // cell, one home.
