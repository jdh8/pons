...
pub fn stayman_minor_slam_try() {
    ...
    assert_eq!(
        best_with(&on, &after_2h, "AJ54.32.32.AKQ32"),
        bid(3, Strain::Clubs)
    );
    ...
    assert_eq!(
        best_with(&on, &after_3c, "A2.AQJ2.K32.Q543"),
        bid(4, Strain::Clubs)
    );
    // No club fit (3♣): sign off in 3NT even with a maximum.
    assert_eq!(
        best_with(&on, &after_3c, "A2.AQJ2.K432.Q54"),
