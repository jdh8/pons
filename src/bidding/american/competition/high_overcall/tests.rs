...
fn high_overcalls_get_a_structure() {
    ...
    let (neg, floored) = best_call_with(&arm, &auction, "K5.KQ54.965.A432");
    assert_eq!(neg, Call::Double, "the 3-level negative double");
    assert!(!floored, "an authored node, not the floor");
    let (game, _) = best_call_with(&arm, &auction, "K5.KQ54.A65.A432");
    ...
    let (major, _) = best_call_with(&arm, &answer, "AQ542.KJ54.96.32");
