...
use super::two_over_one::two_over_one_neural;
...
///
/// Our own distilled net floor ([`two_over_one_neural`]) bound for self-play:
/// the rollout assumes all four seats bid as we would.  This is the policy M3.2
/// iterates — "feed the improved net back into the continuations" — so the live
/// search bidder and its offline distillation share one notion of "how the
/// auction finishes".  Built once and shared across decisions.
static POLICY: LazyLock<Stance> = LazyLock::new(|| two_over_one_neural().against(Family::NATURAL));
