...
fn wrong_hand_does_not_fill_scoped_trick_cache() {
    ...
    // The inference read happens once, and the *trick* cache — the hand-scoped
    // one this test is about — stays empty for a foreign hand.
...
/// Nets and direct consumers share the same memoized authored reading.
///
...
fn nets_share_the_honest_ceiling_reading() {
    ...
    let hand = test_hand();
    let partner_points = |ceilings: bool| {
    ...
    assert_eq!(partner_points(false), Range::new(6, 37));
    // Ceilings on: the truth carries the eight.
    assert_eq!(partner_points(true), Range::new(6, 8));
}
