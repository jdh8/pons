...
pub struct DefenseKnobs {
    ...
    ///
    /// **On by default.**  An exactly six-card major with
    ...
    ///
    /// Shipped after the shared BEN/BBA `Defensive/book/round-1` residual won
    /// or washed against both references and SD-PD did not refute it. `false`
...
pub struct TheirDisclosures {
    ...
    /// 2026-08-15 is the N1j BBA-ladder hybrid ([`CompetitionKnobs::defense_2c_landy_bba`]),
    /// with the legacy cue/transfer stack and the Kokish–Kraft variant
    /// ([`CompetitionKnobs::defense_2c_landy_kk`]) as the other two arms.  The
    /// values `X` and the weak natural `2♦` are common to all three; nothing
    /// else is, so this comment deliberately does not describe a table.
    /// Undeclared (`false`)
