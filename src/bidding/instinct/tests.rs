...
#[test]
fn redoubled_runouts_read_the_pinned_notrump_defense() {
    let advancer_auction = [call(1, Strain::Notrump), Call::Double, Call::Redouble];
    let doubler_auction = [
        call(1, Strain::Notrump),
        Call::Double,
        Call::Redouble,
    ...
    let context = |auction| Context::new(RelativeVulnerability::NONE, auction);

    assert!(advancer_xx_runout_now(&context(&advancer_auction)));
    assert!(doubler_xx_runout_now(&context(&doubler_auction)));
    ...
    for defense in [
        ...
        let mut agreements = Agreements::default();
        agreements.decision.reading.notrump_defense = defense;
        assert!(!advancer_xx_runout_now(
            &context(&advancer_auction).with_profile(agreements.decision)
        ));
        assert!(!doubler_xx_runout_now(
            &context(&doubler_auction).with_profile(agreements.decision)
        ));
    }
}
