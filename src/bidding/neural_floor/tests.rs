...
/// The alert exemption spares their **artificial** game from the game-pull
/// rail and still masks a natural one
///
/// Their keycard reply `1♠ - - 2NT - - 3♠ - - 4NT - - 5♣` is a code; their
/// Smolen landing `4♥` is alerted as a completion but shows hearts, so it is
/// a contract.
/// Read through the bound partnership, whose table-alert reading is what
/// marks the call alerted and its per-call union the length.
#[test]
fn the_game_pull_alert_exemption_spares_only_codes() {
    let pass = Call::Pass;
    let keycard = [
        call(1, Strain::Spades),
        pass,
    ...
    let five_hearts = call(5, Strain::Hearts);
    let smolen = [
        call(1, Strain::Notrump),
        pass,
        call(2, Strain::Clubs),
    ...
    let four_spades = call(4, Strain::Spades);
    let classify = |agreements: &Agreements, auction: &[Call]| {
        let floor = ConfiguredFloorV6::new(
            crate::bidding::features::CompactConfig::symmetric(
                &crate::bidding::features::ConventionCard::capture(agreements),
            ),
        ...
        floor.classify(
            "T87.65.J8542.873".parse().expect("valid test hand"),
            &context,
        )
    };
    let mut agreements = Agreements::default();
    assert!(
        !agreements.decision.instinct.their_game_pull_alert_exempt,
        "the exemption ships default off"
    );
    assert_eq!(
        classify(&agreements, &keycard).0[five_hearts],
        f32::NEG_INFINITY,
        "knob-off the rail masks their keycard reply too"
    );

    agreements.decision.instinct.their_game_pull_alert_exempt = true;
    assert!(
        classify(&agreements, &keycard).0[five_hearts].is_finite(),
        "their keycard reply is left to the net"
    );
    assert_eq!(
        classify(&agreements, &smolen).0[four_spades],
        f32::NEG_INFINITY,
        "their natural game is still masked"
    );
}
