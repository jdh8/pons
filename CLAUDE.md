...
| A knob A/B moves the floor's regime input, or a floor swap needs its siblings moved | [docs/ai-bidder/card-manifold.md](docs/ai-bidder/card-manifold.md) — the shipped v5 floor reads **`Agreements`, not card rows**: 13 of 28 slots per side are trained, the rest folded to zero (so their A/Bs price the book alone). Holds the v4 frozen-coordinate story, the bias fold, and the sibling-factory rule — a system name must reach the same net on its declared and undeclared paths |
