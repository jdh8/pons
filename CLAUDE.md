...
| The Watermelon Dutch Doubleton knobs (5542, the wide non-forcing 1♣ + `1♦!` relay, Odwrotka, the Multi `2♦`) | The book **is** the spec: <https://jdh8.github.io/watermelon-dutch/> (raw markdown in the `jdh8/watermelon-dutch` repo); code in `src/bidding/american/{openings,wide_one_club,multi}.rs` and `rebids/odwrotka.rs`, knobs `opening.five_five_four_two` / `opening.wide_one_club` / `rebid.odwrotka` / `opening.multi_two_diamonds`, all default off with A/Bs owed. The retired `dutch()` system's ledger and verdicts: [docs/archive/dutch-system.md](docs/archive/dutch-system.md) |
