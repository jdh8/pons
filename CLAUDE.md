...
| A **minor-suit transfer** in any lane — ours, a counter-table's — or anything above its completion | [docs/minor-transfer-slam.md](docs/minor-transfer-slam.md) — the cross-lane census: every lane tops out at `3NT` or a placed `5m`, the one `4m` slam try lives in the Landy counter, and its answer is deliberately the floor's |
